...
TEST(ToProtoMessagesTest, EmptySlice) {
  auto slice = test::EmptyDataSlice(0, internal::DataItem(schema::kObject));
  koladata::testing::ExampleMessage message;
  EXPECT_THAT(ToProtoMessages(slice, &message), IsOkAndHolds(IsEmpty()));
}

// Similar to ToProtoTest's AllFieldsAllExtensionsRoundTrip above.
TEST(ToProtoMessagesTest, AllFieldsAllExtensionsRoundTrip) {
  koladata::testing::ExampleMessage message1;
  // Note: 18446744073709551606 = 2**64 - 10, to test uint64 round-trip.
  CHECK(TextFormat::ParseFromString(
      R"pb(
  ...
  ASSERT_OK_AND_ASSIGN(auto output_messages, ToProtoMessages(slice, &message1));

  ASSERT_EQ(output_messages.size(), 3);

  EXPECT_TRUE(MessageDifferencer::Equals(*output_messages[0], message1));
  EXPECT_TRUE(MessageDifferencer::Equals(*output_messages[1], message2));
  EXPECT_TRUE(MessageDifferencer::Equals(*output_messages[2], message3));
}
