...
absl::StatusOr<AttrNamesSet> GetAttrsFromDataItem(
  ...
  if (ds_schema == schema::kSchema) {
    ...
    ASSIGN_OR_RETURN(schema_item,
                     db_impl.GetAttr(item, schema::kSchemaAttr, fallbacks));
  } else {
    // Empty set.
    return AttrNamesSet();
  }
...
absl::StatusOr<AttrNamesSet> GetAttrsFromDataSlice(
  ...
  if (ds_schema == schema::kSchema) {
    ...
    ASSIGN_OR_RETURN(
        schemas, db_impl.GetAttr(internal::DataSliceImpl::Create(*objects_only),
                                 schema::kSchemaAttr, fallbacks));
  } else {
    // Empty set.
    return AttrNamesSet();
  }
...
absl::StatusOr<DataSlice> DataSlice::GetFromDict(const DataSlice& keys) const {
  ...
  BroadcastHelper expanded_keys(keys, shape);
...
void BroadcastHelper::InitWithExpansionSlow(
    const DataSlice& slice, const DataSlice::JaggedShapePtr& shape) {
  auto status_or = slice.BroadcastToShape(shape);
  if (status_or.ok()) {
    owned_expanded_.emplace(std::move(*status_or));
    ptr_ = &(*owned_expanded_);
  } else {
