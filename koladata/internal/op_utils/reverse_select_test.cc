...
TEST(ReverseSelectTest, NoDimensions) {
  ASSERT_OK_AND_ASSIGN(auto edge, CreateJaggedShape({}));
  ASSERT_OK_AND_ASSIGN(
      auto res, ReverseSelectOp()(DataItem(), edge, DataItem(kMissing), edge));
  ASSERT_FALSE(res.has_value());

  ASSERT_OK_AND_ASSIGN(
      res, ReverseSelectOp()(DataItem(1), edge, DataItem(kPresent), edge));
  ASSERT_TRUE(res.holds_value<int>());
  ASSERT_EQ(res.value<int>(), 1);
}

TEST(ReverseSelectTest, NoDimensionsWithError) {
  ASSERT_OK_AND_ASSIGN(auto edge, CreateJaggedShape({}));
  EXPECT_THAT(
      ReverseSelectOp()(DataItem(1), edge, DataItem(kMissing), edge),
      StatusIs(
  ...
  EXPECT_THAT(
      ReverseSelectOp()(DataItem(), edge, DataItem(kPresent), edge),
      StatusIs(
          absl::StatusCode::kInvalidArgument,
          "it is not possible to get the provided DataSlice after applying the "
          "filter because both are DataItems but have different presences."));
  EXPECT_THAT(
      ReverseSelectOp()(DataItem(1), edge, DataItem(1), edge),
      StatusIs(absl::StatusCode::kInvalidArgument,
               "second argument to operator reverse_select must have all items "
               "of MASK dtype"));
}

TEST(ReverseSelectTest, OneDimension) {
  ...
  // Int
  {
    ...
    ASSERT_OK_AND_ASSIGN(auto res,
                         ReverseSelectOp()(ds, ds_edge, filter, filter_edge));
...
TEST(ReverseSelectTest, OneDimensionWithEmptyFilter) {
...
TEST(ReverseSelectTest, OneDimensionWithError) {
  ...
  // Invalid shape
  {
    ...
    EXPECT_THAT(
        ReverseSelectOp()(ds, ds_edge, filter, filter_edge),
        StatusIs(absl::StatusCode::kInvalidArgument,
                 "it is not possible to get the provided DataSlice after "
                 "applying the filter because the shape of the DataSlice "
                 "is different from the shape after applying the filter. "
                 "The shape of the DataSlice: JaggedShape(1), the shape "
                 "after applying the filter: JaggedShape(2)"));
  }
}

TEST(ReverseSelectTest, TwoDimensions) {
...
TEST(ReverseSelectTest, ThreeDimensionsWithError) {
  auto ds = DataSliceImpl::Create(CreateDenseArray<int>({1, std::nullopt, 2}));
  ...
  // Last dimension is invalid
  {
    ...
    EXPECT_THAT(
        ...
        StatusIs(
            absl::StatusCode::kInvalidArgument,
            "it is not possible to get the provided DataSlice after applying "
            "the filter because the shape of the DataSlice is different from "
            "the shape after applying the filter. The shape of the DataSlice: "
            "JaggedShape(2, [2, 1], [2, 1, 0]), the shape after applying the "
            "filter: JaggedShape(2, [2, 1], [3, 1, 0])"));
  }
  ...
  // Second dimension is invalid
  {
    ...
    EXPECT_THAT(
        ...
        StatusIs(
            ...
            "the shape after applying the filter. The shape of the DataSlice: "
            "JaggedShape(2, [2, 1], [2, 1, 0]), the shape after applying the "
            "filter: JaggedShape(2, [1, 2], [2, 1, 0])"));
  }
}

TEST(ReverseSelectTest, InvalidCase) {
  {
    ...
    EXPECT_THAT(ReverseSelectOp()(ds, ds_edge, filter, filter_edge),
