...
TEST(TriplesTest, TripleDebugString) {
  ...
  EXPECT_THAT(
      ...
      ::testing::MatchesRegex("ObjectId=4.*\\.3 attr=a value=1"));
  EXPECT_THAT((DictItemTriple{dict, DataItem(arolla::Text("a")), DataItem(1)})
                  .DebugString(),
              ::testing::MatchesRegex("DictId=1.*\\.3 key='a' value=1"));
  EXPECT_THAT((DictItemTriple{schema, DataItem(arolla::Text("a")), DataItem(1)})
                  .DebugString(),
              ::testing::MatchesRegex("SchemaId=f.*\\.0 key='a' value=1"));
}
...
TEST(TriplesTest, SimpleAttr) {
  ...
  ObjectId=4000000000000010.0 attr=a value=4000000000000020.0
  ObjectId=4000000000000010.0 attr=c value='aaa'
  ObjectId=4000000000000020.0 attr=b value=5
  ObjectId=4000000000000020.0 attr=c value=b'bbb'
})DB");
}
