...
class CoreSortTest(parameterized.TestCase):
  ...
  def test_eval_with_sort_by(self, x, sort_by, expected):
    result = expr_eval.eval(kde.core.sort(x, sort_by))
    testing.assert_equal(result, expected)
  ...
  def test_eval_with_descending_unspecified(self, x, expected):
    result = expr_eval.eval(kde.core.sort(x))
    testing.assert_equal(result, expected)
  ...
  def test_sort_by_more_sparse(self):
    with self.assertRaisesRegex(
      ...
      expr_eval.eval(kde.core.sort(ds([0, 3, 6]), ds([2, 1, None])))
  ...
  def test_view(self):
    self.assertTrue(view.has_data_slice_view(kde.core.sort(I.x)))

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.core.sort, kde.sort))
