...
class PyMapPyOnSelectedTest(parameterized.TestCase):
  ...
  def test_same_dimension_cond(self):
    ...
    res = expr_eval.eval(kde.py.map_py_on_selected(yes_fn, val > 2, val))
    ...
    res = expr_eval.eval(
        kde.py.map_py_on_selected(
            yes_fn, kde.logical.has_not(val) | (val > 2), val
        )
    )
    testing.assert_equal(
  ...
  def test_smaller_dimension_cond(self):
    ...
    res = expr_eval.eval(kde.py.map_py_on_selected(yes_fn, cond, val))
    testing.assert_equal(
  ...
  def test_error_non_mask_cond(self):
    ...
    with self.assertRaisesWithLiteralMatch(
      ...
      expr_eval.eval(kde.py.map_py_on_selected(yes_fn, val, val))
  ...
  def test_error_higher_dimension_cond(self):
    ...
    with self.assertRaisesWithLiteralMatch(
      ...
      expr_eval.eval(kde.py.map_py_on_selected(yes_fn, val.add_dim(1) > 2, val))

  def test_view(self):
    self.assertTrue(
        view.has_data_slice_view(kde.py.map_py_on_selected(I.fn, I.cond, I.arg))
    )
