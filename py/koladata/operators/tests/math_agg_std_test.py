...
class MathAggStdTest(parameterized.TestCase):

  @parameterized.parameters(
      # INT32
      (
          ds([10, 9, 11, 12], schema_constants.INT32),
          ds(1.2909944, schema_constants.FLOAT32),
      ),
      # INT64
      (
      ...
      (
          ...
          False,
          ds(1),
          ds(0.8164966, schema_constants.FLOAT32),
      ),
      (
          ds([0, None, -2147483648], schema_constants.FLOAT32),
          False,
          ds(1),
          ds(1073741800.0, schema_constants.FLOAT32),
      ),
      (
          ds([0, None, -2147483648], schema_constants.FLOAT32),
  ...
  def test_eval(self, *args_and_expected):
    ...
    result = expr_eval.eval(kde.math.agg_std(*args))
  ...
  def test_out_of_bounds_ndim_error(self, ndim):
    ...
    with self.assertRaisesRegex(ValueError, 'expected 0 <= ndim <= rank'):
      expr_eval.eval(kde.math.agg_std(x, ndim=ndim))
  ...
  def test_view(self):
    self.assertTrue(view.has_data_slice_view(kde.math.agg_std(I.x)))

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.math.agg_std, kde.agg_std))
