...
class MathAggStdTest(parameterized.TestCase):
  ...
  def test_non_scalar_unbiased_error(self):
    with self.assertRaisesRegex(
      ...
      kd.math.agg_std(ds([1]), unbiased=ds([True]))
  ...
  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        kde.math.agg_std,  # pyrefly: ignore[missing-attribute]
        QTYPES,
        possible_qtypes=qtypes.DETECT_SIGNATURES_QTYPES,  # pyrefly: ignore[bad-argument-type]
    )

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.math.agg_std(I.x)))  # pyrefly: ignore[missing-attribute]
