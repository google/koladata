...
class StringsCountTest(parameterized.TestCase):
  ...
  def test_eval(self, s, substr, expected):
    result = kd.strings.count(s, substr)
  ...
  def test_incompatible_types_error(self):
    with self.assertRaisesRegex(
      ...
      kd.strings.count(ds('foo'), ds(b'f'))
  ...
  def test_another_incompatible_types_error(self):
    with self.assertRaisesRegex(
      ...
      kd.strings.count(ds([None], schema_constants.STRING), ds(123))
  ...
  def test_mixed_slice_error(self):
    with self.assertRaisesRegex(
      ...
      kd.strings.count(ds('foo'), ds([1, 'fo']))

  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        kde.strings.count,  # pyrefly: ignore[missing-attribute]
        QTYPES,
        possible_qtypes=test_qtypes.DETECT_SIGNATURES_QTYPES,  # pyrefly: ignore[bad-argument-type]
    )

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.strings.count(I.x, I.y)))  # pyrefly: ignore[missing-attribute]
