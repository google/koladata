...
class LogicalAggAllTest(parameterized.TestCase):
  ...
  def test_eval(self, *args_and_expected):
    ...
    result = kd.masking.agg_all(*args)
  ...
  def test_out_of_bounds_ndim_error(self, ndim):
    ...
    with self.assertRaisesRegex(ValueError, 'expected 0 <= ndim <= rank'):
      kd.masking.agg_all(x, ndim=ndim)
  ...
  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        kde.masking.agg_all,  # pyrefly: ignore[missing-attribute]
        QTYPES,
        possible_qtypes=qtypes.DETECT_SIGNATURES_QTYPES,  # pyrefly: ignore[bad-argument-type]
    )

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.masking.agg_all(I.x)))  # pyrefly: ignore[missing-attribute]

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.masking.agg_all, kde.agg_all))  # pyrefly: ignore[missing-attribute]
