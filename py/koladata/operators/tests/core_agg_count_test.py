...
class CoreAggCountTest(parameterized.TestCase):
  ...
  def test_eval(self, *args_and_expected):
    ...
    result = expr_eval.eval(kde.core.agg_count(*args))
  ...
  def test_data_item_input_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      expr_eval.eval(kde.core.agg_count(x))
  ...
  def test_out_of_bounds_ndim_error(self, ndim):
    ...
    with self.assertRaisesRegex(ValueError, 'expected 0 <= ndim <= rank'):
      expr_eval.eval(kde.core.agg_count(x, ndim))
  ...
  def test_view(self):
    self.assertTrue(view.has_data_slice_view(kde.core.agg_count(I.x)))

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.core.agg_count, kde.agg_count))
