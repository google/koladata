...
class KodaInternalParallelStreamChainFromStreamTest(parameterized.TestCase):
  ...
  def test_basic(self):
    ...
    res = expr_eval.eval(
        kde_internal.parallel.stream_chain_from_stream(sstream)  # pyrefly: ignore[missing-attribute]
    )
  ...
  def test_no_input_streams(self):
    ...
    swriter.close()
    res = expr_eval.eval(
        kde_internal.parallel.stream_chain_from_stream(sstreams)  # pyrefly: ignore[missing-attribute]
    )
  ...
  def test_error_non_stream_of_streams(self):
    ...
    with self.assertRaisesRegex(
      ...
      kde_internal.parallel.stream_chain_from_stream(stream)  # pyrefly: ignore[missing-attribute]
  ...
  def test_view(self):
    self.assertTrue(
        view.has_koda_view(
            kde_internal.parallel.stream_chain_from_stream(I.sstream)  # pyrefly: ignore[missing-attribute]
        )
    )

  def test_repr(self):
    self.assertEqual(
        repr(kde_internal.parallel.stream_chain_from_stream(I.a)),  # pyrefly: ignore[missing-attribute]
