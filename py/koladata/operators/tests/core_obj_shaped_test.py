...
def generate_qtypes():
  for itemid_arg_type in [DATA_SLICE, arolla.UNSPECIFIED]:
    for attrs_type in [
      ...
      yield JAGGED_SHAPE, itemid_arg_type, attrs_type, arolla.types.INT64, DATA_SLICE
...
class CoreObjShapedTest(absltest.TestCase):
  ...
  def test_view(self):
    self.assertTrue(view.has_data_slice_view(kde.core.obj_shaped(I.x)))
    self.assertTrue(view.has_data_slice_view(kde.core.obj_shaped(I.x, a=I.y)))
