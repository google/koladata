...
class SlicesGroupByIndicesSortedTest(parameterized.TestCase):
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.slices.group_by_indices_sorted(I.x)))

  def test_alias(self):
    self.assertTrue(
        optools.equiv_to_op(
            kde.slices.group_by_indices_sorted, kde.group_by_indices_sorted
        )
    )
