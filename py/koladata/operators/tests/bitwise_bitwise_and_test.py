...
class BitwiseAndTest(parameterized.TestCase):
  ...
  @parameterized.parameters(
      ...
      (ds([5, 5, 5]), ds([3, 5, 0]), ds([1, 5, 0])),
      (ds(5), ds(None), ds(None, schema_constants.INT32)),
      (ds(5), ds([3, 5, 0]), ds([1, 5, 0])),
      (
  ...
  def test_eval(self, x, y, expected):
    result = expr_eval.eval(kde.bitwise.bitwise_and(I.x, I.y), x=x, y=y)
  ...
  @parameterized.parameters(
      (
          ...
          ds(1),
          (
              'kd.bitwise.bitwise_and: argument `x` must be a slice of integer'
      ...
      (
          ...
          ds(3.0),
          (
              'kd.bitwise.bitwise_and: argument `y` must'
  ...
  def test_errors(self, x, y, expected_error):
    with self.assertRaisesWithLiteralMatch(ValueError, expected_error):
      expr_eval.eval(kde.bitwise.bitwise_and(I.x, I.y), x=x, y=y)
