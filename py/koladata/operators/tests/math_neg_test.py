...
class MathNegTest(parameterized.TestCase):
  ...
  def test_eval_numeric(self, x, expected):
    result = expr_eval.eval(kde.math.neg(I.x), x=x)
  ...
  def test_errors(self):
    ...
    with self.assertRaisesRegex(
      ...
      expr_eval.eval(kde.math.neg(I.x), x=x)
  ...
  def test_repr(self):
    ...
    self.assertEqual(repr(kde.neg(I.x)), '-I.x')

  def test_view(self):
    self.assertTrue(view.has_data_slice_view(kde.math.neg(I.x)))

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.math.neg, kde.neg))
