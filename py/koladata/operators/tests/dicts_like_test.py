...
class DictLikeTest(parameterized.TestCase):
  ...
  @parameterized.parameters(
      ...
      # value_schema arg
      (ds([5, 6]), dict(value_schema=schema_constants.INT64)),
      # schema arg
      (
          ds([5, 6]),
          dict(
      ...
      # itemid arg
      (ds([5, 6]), dict(itemid=bag().dict_like(ds([5, 6])).get_itemid())),
  )
  def test_value(self, shape_and_mask_from, kwargs):
    actual = kd.dicts.like(shape_and_mask_from, **kwargs)
  ...
  def test_keys_values(self):
    ...
    keys = ds(['a', 'b'])
    values = ds([3, 7])
    actual = kd.dicts.like(shape_and_mask_from, keys=keys, values=values)
    expected = bag().dict_like(
        shape_and_mask_from, items_or_keys=keys, values=values
    )
    testing.assert_equivalent(actual, expected)
  ...
  def test_sparsity(self):
    ...
    keys = ds(['a', 'b', 'c'])
    values = ds([3, 7, 8])
    actual = kd.dicts.like(shape_and_mask_from, keys, values)
    expected = bag().dict_like(shape_and_mask_from, keys, values)
    testing.assert_equivalent(actual, expected)

  def test_db_is_immutable(self):
    d = kd.dicts.like(ds(5))
  ...
  def test_itemid_dataitem(self):
    ...
    with self.subTest('present DataItem and missing itemid'):
      with self.assertRaisesRegex(
        ...
        _ = kd.dicts.like(ds(1), 'a', 42, itemid=(itemid & None))

  def test_itemid_dataslice(self):
    id1 = kd.allocation.new_dictid()
    id2 = kd.allocation.new_dictid()
    id3 = kd.allocation.new_dictid()

    with self.subTest('full DataSlice and full itemid'):
      x = kd.dicts.like(ds([1, 1, 1]), 'a', 42, itemid=ds([id1, id2, id3]))
      testing.assert_equal(
          x,
    ...
    with self.subTest('sparse DataSlice and full itemid'):
      x = kd.dicts.like(ds([1, None, 1]), 'a', 42, itemid=ds([id1, id2, id3]))
      testing.assert_equal(
          x,
          ds([id1, None, id3])
          .with_schema(x.get_schema())
    ...
    with self.subTest('sparse DataSlice and full itemid with duplicates'):
      with self.assertRaisesRegex(
        ...
        _ = kd.dicts.like(ds([1, None, 1]), 'a', 42, itemid=ds([id1, id1, id1]))

    with self.subTest(
        'sparse DataSlice and full itemid with unused duplicates'
    ):
      x = kd.dicts.like(ds([1, None, 1]), 'a', 42, itemid=ds([id1, id1, id3]))
      testing.assert_equal(
          x,
          ds([id1, None, id3])
          .with_schema(x.get_schema())
  ...
  def test_itemid_from_different_bag(self):
    ...
    # Successful.
    x = kd.dicts.like(ds([[1, None], [1]]), itemid=itemid.get_itemid())
  ...
  def test_wrong_shape_and_mask_from(self):
    with self.assertRaisesRegex(
        ValueError, 'expected DATA_SLICE, got shape_and_mask_from: JAGGED_SHAPE'
    ):
      kd.dicts.like(jagged_shape.create_shape([2]))
  ...
  def test_only_keys_arg_error(self):
    with self.assertRaisesRegex(
      ...
      kd.dicts.like(ds([5, 6]), keys=ds(['a', 'b']))
  ...
  def test_wrong_arg_types(self):
    ...
    with self.assertRaisesRegex(
      ...
      kd.dicts.like(shape_from, key_schema=42)
    with self.assertRaisesRegex(
        ValueError, "schema's schema must be SCHEMA, got: INT32"
    ):
      kd.dicts.like(shape_from, value_schema=42)
    with self.assertRaisesRegex(
        ValueError, "schema's schema must be SCHEMA, got: INT32"
    ):
      kd.dicts.like(shape_from, schema=42)
...
Assigned schema for keys: STRING""",
  ...
  def test_non_determinism(self):
    ...
    keys = ds([2, 3])
    values = ds([3, 7])
    res_1 = kd.dicts.like(shape_and_mask_from, keys=keys, values=values)
    res_2 = kd.dicts.like(shape_and_mask_from, keys=keys, values=values)
    self.assertNotEqual(
