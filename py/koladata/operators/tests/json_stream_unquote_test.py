...
class JsonStreamUnquoteTest(parameterized.TestCase):
  ...
  def test_parallel_transform(self):
    ...
    result = parallel_fns.transform(
        functor_factories.expr_fn(returns=kde.json_stream.unquote(I.x)),  # pyrefly: ignore[missing-attribute]
    )(
        executor,
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.json_stream.unquote(I.x)))  # pyrefly: ignore[missing-attribute]
