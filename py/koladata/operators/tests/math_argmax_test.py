...
class MathArgminTest(parameterized.TestCase):
  ...
  def test_eval(self, *args_and_expected):
    ...
    result = expr_eval.eval(kde.math.argmax(*args))
  ...
  def test_data_item_input_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      expr_eval.eval(kde.math.argmax(x))
  ...
  def test_out_of_bounds_ndim_error(self, ndim):
    ...
    with self.assertRaisesRegex(ValueError, 'expected 0 <= ndim <= rank'):
      expr_eval.eval(kde.math.argmax(x, ndim=ndim))
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.math.argmax(I.x)))

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.math.argmax, kde.argmax))
