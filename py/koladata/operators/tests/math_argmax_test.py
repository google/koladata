...
class MathArgmaxTest(parameterized.TestCase):
  ...
  @parameterized.parameters(
      ...
      (ds([None], schema_constants.INT32), ds(None, schema_constants.INT64)),
      (
          ds([[1, None], [3, 4], [None, None], []]),
          ds([0, 1, None, None], schema_constants.INT64),
      ),
      (
          ds([[[1, None], [3, 4]], [[None, None]]]),
          ds([[0, 1], [None]], schema_constants.INT64),
      ),
      (
          ds(
              [[1.0, None], [3.999999, 4.0], [None, None], []],
      ...
      (
          ds(
              [[1.0, None], [3.9999999, 4.0], [None, None], []],
      ...
      (
          ds([[1, None, None], [3, 4], [None, None]]),
          ds(0),
          ds([[0, None, None], [0, 0], [None, None]], schema_constants.INT64),
      ),
      # Nans
      (
          ds([[3.0, 1.0, np.nan, None], [None, np.nan, np.nan, None]]),
          ds([2, 1], schema_constants.INT64),
      ),
      # OBJECT
      (
          ds([[2, None], [None]], schema_constants.OBJECT),
          ds([0, None], schema_constants.INT64),
      ),
  ...
  def test_entity_slice_error(self):
    ...
    with self.assertRaisesRegex(
        ValueError,
        re.escape(
            'kd.math.argmax: argument `x` must be a slice of numeric values,'
  ...
  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        kde.math.argmax,  # pyrefly: ignore[missing-attribute]
        QTYPES,
        possible_qtypes=qtypes.DETECT_SIGNATURES_QTYPES,  # pyrefly: ignore[bad-argument-type]
    )

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.math.argmax(I.x)))  # pyrefly: ignore[missing-attribute]

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.math.argmax, kde.argmax))  # pyrefly: ignore[missing-attribute]
