...
class LogicalAnyTest(parameterized.TestCase):
  ...
  def test_eval(self, *args_and_expected):
    ...
    result = expr_eval.eval(kde.masking.any(*args))
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.masking.any(I.x)))

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.masking.any, kde.any))
