...
class CoreStackTest(parameterized.TestCase):
  ...
  def test_eval(self, args, ndim, expected):
    result = expr_eval.eval(kde.core.stack(*args, ndim=ndim))
  ...
  def test_invalid_ndim(self, args, ndim, expected_regex):
    with self.assertRaisesRegex(ValueError, expected_regex):
      expr_eval.eval(kde.core.stack(*args, ndim=ndim))

  def test_same_databag(self):
    db = data_bag.DataBag.empty()
    a = db.obj(x=1)
    b = db.obj(x=2)
    result = expr_eval.eval(kde.core.stack(a, b))
