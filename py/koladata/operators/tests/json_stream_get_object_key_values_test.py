...
class JsonStreamGetObjectKeyValuesTest(parameterized.TestCase):
  ...
  def test_parallel_transform(self):
    ...
    result = parallel_fns.transform(
        functor_factories.expr_fn(
            returns=kde.json_stream.get_object_key_values(I.x, key='x')
        ),
    )(
        executor,
  ...
  def test_bad_arguments(self):
    ...
    with self.assertRaisesRegex(
        ValueError,
        re.escape(
            'kd.json_stream._get_object_key_values_stream: argument `key` must'
  ...
  def test_view(self):
    self.assertTrue(
        view.has_koda_view(kde.json_stream.get_object_key_values(I.x, key='x'))
    )
