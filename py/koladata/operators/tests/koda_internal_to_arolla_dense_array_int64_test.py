...
"""Tests for koda_internal.to_arolla_dense_array_int64 operator."""
...
class KodaToArollaDenseArrayInt64Test(parameterized.TestCase):
  ...
  def test_eval(self, x, expected):
    testing.assert_equal(kd_internal.to_arolla_dense_array_int64(x), expected)
  ...
  def test_unsupported_schema_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      kd_internal.to_arolla_dense_array_int64(x)
  ...
  def test_unsupported_entity(self):
    with self.assertRaisesRegex(ValueError, 'common schema'):
      kd_internal.to_arolla_dense_array_int64(bag().new(x=[1]))

  def test_unsupported_object(self):
    with self.assertRaisesRegex(ValueError, 'common schema'):
      kd_internal.to_arolla_dense_array_int64(bag().obj(x=[1]))

  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        kde_internal.to_arolla_dense_array_int64,  # pyrefly: ignore[missing-attribute]
        [(qtypes.DATA_SLICE, arolla.DENSE_ARRAY_INT64)],
        possible_qtypes=qtypes.DETECT_SIGNATURES_QTYPES,  # pyrefly: ignore[bad-argument-type]
    )

  def test_view(self):
    self.assertFalse(
        view.has_koda_view(kde_internal.to_arolla_dense_array_int64(I.x))  # pyrefly: ignore[missing-attribute]
    )
