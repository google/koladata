...
"""Tests for kde.schema.get_key_schema."""
...
dict_s1 = db.dict_schema(schema_constants.INT32, schema_constants.TEXT)
dict_s2 = db.dict_schema(dict_s1, schema_constants.TEXT)
...
class SchemaGetKeySchemaTest(parameterized.TestCase):
  ...
  def test_eval(self, x, expected):
    res = expr_eval.eval(kde.schema.get_key_schema(x))
  ...
  def test_non_dict_schema(self):
    with self.assertRaisesRegex(
      ...
      expr_eval.eval(kde.schema.get_key_schema(db.new(x=1)))

    with self.assertRaisesRegex(
        ValueError,
        "expected Dict schema for get_key_schema",
    ):
      expr_eval.eval(kde.schema.get_key_schema(schema_constants.ANY))
  ...
  def test_view(self):
    self.assertTrue(view.has_data_slice_view(kde.schema.get_key_schema(I.x)))
