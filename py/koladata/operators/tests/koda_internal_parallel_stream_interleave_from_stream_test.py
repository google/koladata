...
class KodaInternalParallelStreamInterleaveFromStreamTest(
  ...
  def test_error_on_stream(self):
    ...
    for _ in range(5):
      ...
      swriter.write(stream)
      self.assertEqual(reader.read_available(), [])
    writers[2].close(ValueError('Boom!'))
  ...
  def test_view(self):
    self.assertTrue(
        view.has_koda_view(
            koda_internal_parallel.stream_interleave_from_stream(I.sstream)
        )
    )

  def test_repr(self):
    self.assertEqual(
        repr(koda_internal_parallel.stream_interleave_from_stream(I.a)),
