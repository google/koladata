...
class SchemaIsNamedSchemaTest(parameterized.TestCase):
  ...
  @parameterized.parameters(
      ...
      (
          bag.named_schema('bar').repeat(2),
          mask_constants.missing,
      ),
      (bag.new(x=1), mask_constants.missing),
  )
  def test_eval(self, x, expected_result):
    testing.assert_equal(kd.schema.is_named_schema(x), expected_result)

  def test_qtype_signature(self):
    arolla.testing.assert_qtype_signatures(
        kde.schema.is_named_schema,  # pyrefly: ignore[missing-attribute]
        [(DATA_SLICE, DATA_SLICE)],
        possible_qtypes=qtypes.DETECT_SIGNATURES_QTYPES,  # pyrefly: ignore[bad-argument-type]
    )

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.schema.is_named_schema(I.x)))  # pyrefly: ignore[missing-attribute]
