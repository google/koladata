...
class FunctorCallAndUpdateNamedTupleTest(absltest.TestCase):
  ...
  def test_simple(self):
    ...
    testing.assert_equal(
        kd.functor.call_and_update_namedtuple(
            fn, x=2, namedtuple_to_update=kd.namedtuple(x=1, y=2)
        ),
    ...
    testing.assert_equal(
        kd.functor.call_and_update_namedtuple(
            fn, x=2, y=3, namedtuple_to_update=kd.namedtuple(y=1, x=2)
        ),
  ...
  def test_non_named_tuple_returned(self):
    ...
    with self.assertRaisesRegex(
      ...
      _ = kd.functor.call_and_update_namedtuple(
          fn, x=2, namedtuple_to_update=kd.namedtuple(x=1, y=2)
      )
  ...
  def test_non_named_tuple_to_update(self):
    ...
    with self.assertRaisesRegex(
      ...
      _ = kd.functor.call_and_update_namedtuple(
          fn, x=2, namedtuple_to_update=kd.tuple(1, 2)
      )
  ...
  def test_databag_values(self):
    ...
    res = kd.functor.call_and_update_namedtuple(
        fn,
        x=x,
        namedtuple_to_update=kd.namedtuple(
            a=kd.attrs(x, foo=2), b=kd.attrs(x, foo=3)
        ),
    )
