...
class CoreRemoveTest(parameterized.TestCase):
  ...
  def test_eval(self, values, filter_arr, expected):
    result = expr_eval.eval(kde.slices.remove(values, filter_arr))
  ...
  def test_remove_wrong_filter_schema(self):
    ...
    with self.assertRaisesRegex(
        ValueError,
        'kde.slices.remove: argument `fltr` must have kd.MASK dtype',
    ):
      expr_eval.eval(kde.slices.remove(val, val))
  ...
  def test_remove_expand_to_shape(self):
    ...
    result = expr_eval.eval(kde.slices.remove(x, y))
  ...
  def test_remove_expand_to_shape_fails(self):
    ...
    with self.assertRaisesRegex(
      ...
      _ = expr_eval.eval(kde.slices.remove(x, y))

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.slices.remove(I.x, I.y)))

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.slices.remove, kde.remove))
