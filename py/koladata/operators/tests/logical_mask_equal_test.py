...
class LogicalMaskEqualTest(parameterized.TestCase):
  ...
  def test_eval(self, x, y, expected):
    result = expr_eval.eval(kde.logical.mask_equal(x, y))
  ...
  def test_invalid_input(self):
    ...
    with self.assertRaisesRegex(
      ...
      _ = expr_eval.eval(kde.logical.mask_equal(ds(1), ds(present)))

    with self.assertRaisesRegex(
        ValueError, re.escape('`y` must have kd.MASK dtype.')
    ):
      _ = expr_eval.eval(kde.logical.mask_equal(ds(present), ds(1)))
  ...
  def test_view(self):
    self.assertTrue(view.has_data_slice_view(kde.logical.mask_equal(I.x, I.y)))

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.logical.mask_equal, kde.mask_equal))
