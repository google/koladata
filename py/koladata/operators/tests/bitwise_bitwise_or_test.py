...
class BitwiseOrTest(parameterized.TestCase):
  ...
  def test_eval(self, x, y, expected):
    result = kd.bitwise.bitwise_or(x, y)
  ...
  def test_qtype_signatures(self):
    self.assertCountEqual(
        arolla.testing.detect_qtype_signatures(
            kde.bitwise.bitwise_or,  # pyrefly: ignore[missing-attribute]
  ...
  def test_repr(self):
    self.assertEqual(
        repr(kde.bitwise.bitwise_or(I.x, I.y)),  # pyrefly: ignore[missing-attribute]
        'kd.bitwise.bitwise_or(I.x, I.y)',
    )

  def test_alias(self):
    self.assertTrue(
        optools.equiv_to_op(kde.bitwise.bitwise_or, kde.bitwise_or)  # pyrefly: ignore[missing-attribute]
    )

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.bitwise.bitwise_or(I.x, I.y)))  # pyrefly: ignore[missing-attribute]
