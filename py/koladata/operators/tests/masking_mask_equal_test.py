...
class LogicalMaskEqualTest(parameterized.TestCase):
  ...
  def test_eval(self, x, y, expected):
    result = kd.masking.mask_equal(x, y)
  ...
  def test_invalid_input(self):
    ...
    with self.assertRaisesRegex(
      ...
      _ = kd.masking.mask_equal(ds(1), ds(present))

    with self.assertRaisesRegex(
        ValueError,
        re.escape('argument `y` must be a slice of MASK, got a slice of INT32'),
    ):
      _ = kd.masking.mask_equal(ds(present), ds(1))

  def test_qtype_signatures(self):
    self.assertCountEqual(
        arolla.testing.detect_qtype_signatures(
            kde.masking.mask_equal,  # pyrefly: ignore[missing-attribute]
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.masking.mask_equal(I.x, I.y)))  # pyrefly: ignore[missing-attribute]

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.masking.mask_equal, kde.mask_equal))  # pyrefly: ignore[missing-attribute]
