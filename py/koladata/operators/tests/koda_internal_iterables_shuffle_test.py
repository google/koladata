...
class IterablesInternalShuffleTest(absltest.TestCase):
  ...
  def test_basic(self):
    ...
    res = expr_eval.eval(koda_internal_iterables.shuffle(I.arg), arg=a)
    ...
    self.assertEqual(
        frozenset(
            arolla.testing.detect_qtype_signatures(
                koda_internal_iterables.shuffle,
                possible_qtypes=[
                    qtypes.DATA_SLICE,
                    qtypes.DATA_BAG,
                    arolla.INT32,
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(koda_internal_iterables.shuffle(I.arg)))
