...
class JsonStreamGetObjectKeyValueTest(parameterized.TestCase):

  def test_basic(self):
    result = kd.json_stream.get_object_key_value(
        kd.iterables.make('{"x":1,"y":2}\n'), key='x'
    )
  ...
  def test_parallel_transform(self):
    ...
    result = parallel_fns.transform(
        functor_factories.expr_fn(
            returns=kde.json_stream.get_object_key_value(I.x, key='x')  # pyrefly: ignore[missing-attribute]
        ),
    )(
        executor,
        x=kd.streams.make('{"x":1,"y":2}\n'),
  ...
  def test_bad_arguments(self):
    with self.assertRaisesRegex(
      ...
      kd.json_stream.get_object_key_value(kd.iterables.make(123), key='x')
    with self.assertRaisesRegex(
        ValueError,
        re.escape(
            'expected STRING DataItem from input stream, got slice with schema'
    ...
    with self.assertRaisesRegex(
      ...
      kd.json_stream.get_object_key_value(kd.iterables.make(), key=123)
    with self.assertRaisesRegex(
        ValueError,
        re.escape(
  ...
  def test_view(self):
    self.assertTrue(
        view.has_koda_view(kde.json_stream.get_object_key_value(I.x, key='x'))  # pyrefly: ignore[missing-attribute]
    )
