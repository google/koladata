...
class ProtoToProtoBytesTest(parameterized.TestCase):

  def test_none(self):
    result = expr_eval.eval(
        kde.proto.to_proto_bytes(None, 'koladata.functions.testing.MessageA')
    )
  ...
  def test_empty(self):
    ...
    self.assertEqual(result.to_py(), b'')

  def test_nonempty(self):
    result = expr_eval.eval(
        kde.proto.to_proto_bytes(
            fns.new(some_text='xyz'), 'koladata.functions.testing.MessageA',
        )
    )
    self.assertEqual(
        result.to_py(), test_pb2.MessageA(some_text='xyz').SerializeToString()
    )
  ...
  def test_nonempty_extension(self):
    ...
    self.assertEqual(result.to_py(), m.SerializeToString())
  ...
  def test_sparse_jagged_input(self):
    ...
    self.assertEqual(result.to_py(), [[b'', b''], [b'', None, b'']])

  def test_proto_path_not_found(self):
    with self.assertRaisesRegex(
        exceptions.KodaError,
        re.escape(
            'kd.proto.to_proto_bytes: proto message `not.a.Message` not'
            ' found in C++ generated descriptor pool'
        ),
    ):
      expr_eval.eval(kde.proto.to_proto_bytes(fns.new(), 'not.a.Message'))
