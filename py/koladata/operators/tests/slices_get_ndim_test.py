...
class SlicesRankTest(parameterized.TestCase):
  ...
  def test_eval(self, *args_and_expected):
    ...
    result = kd.slices.get_ndim(*args)
    testing.assert_equal(result, expected_value)

  def test_qtype_signatures(self):
    self.assertCountEqual(
        arolla.testing.detect_qtype_signatures(
            kde.slices.get_ndim,  # pyrefly: ignore[missing-attribute]
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.slices.get_ndim(I.x)))  # pyrefly: ignore[missing-attribute]

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.slices.get_ndim, kde.get_ndim))  # pyrefly: ignore[missing-attribute]
