...
class AssertionAssertPrimitiveTest(parameterized.TestCase):
  ...
  def test_eval(self, arg_name, x, dtype):
    result = kd.assertion.assert_primitive(arg_name, x, dtype)
  ...
  def test_assertion_fails(self, x, dtype):
    with self.assertRaisesRegex(
      ...
      kd.assertion.assert_primitive('foo', x, dtype)
  ...
  def test_invalid_dtype(self):
    with self.assertRaisesRegex(
      ...
      kd.assertion.assert_primitive('unused', ds(1), schema_constants.OBJECT)

    with self.assertRaisesRegex(
        ValueError,
        "primitive schema's schema must be SCHEMA",
    ):
      kd.assertion.assert_primitive('unused', ds(1), ds(1))
