...
class SlicesSliceTest(parameterized.TestCase):
  ...
  def test_eval_single_arg(self, x, expected):
    res = expr_eval.eval(kde.slices.slice(x))  # pyrefly: ignore[missing-attribute]
  ...
  def test_eval_two_args(self, x, schema, expected):
    res = expr_eval.eval(kde.slices.slice(x, schema))  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(res, expected)
    res = expr_eval.eval(kde.slices.slice(x, schema=schema))  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(res, expected)
  ...
  def test_cast_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      expr_eval.eval(kde.slices.slice(x, schema_constants.INT32))  # pyrefly: ignore[missing-attribute]

  def test_expr_argument(self):
    x = kde.uu(a=ds([1, 2, 3]))  # pyrefly: ignore[missing-attribute]
    res = expr_eval.eval(kde.slices.slice(x))  # pyrefly: ignore[missing-attribute]
  ...
  def test_expr_argument_inside_list(self):
    x = [1, kde.uuobj(a=2), 3]  # pyrefly: ignore[missing-attribute]
    res = expr_eval.eval(kde.slices.slice(x))  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(
        res.no_bag(), ds([1, expr_eval.eval(kde.uuobj(a=2)).no_bag(), 3])  # pyrefly: ignore[missing-attribute]
    )
    testing.assert_equal(res.S[1].a.no_bag(), ds(2).no_bag())

  def test_expr_argument_inside_list_with_cast(self):
    x = [1, I.x, 3]
    res = expr_eval.eval(kde.slices.slice(x, schema_constants.INT64), x=2.1)  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(res, ds([1, 2, 3], schema_constants.INT64))

  def test_expr_argument_inside_nested_list(self):
    x = [[kde.obj(a=I.x), kde.obj(a=I.y)], [kde.obj(a=I.z)]]  # pyrefly: ignore[missing-attribute]
    res = expr_eval.eval(kde.slices.slice(x), x=1, y=2, z=3)  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(res.a.no_bag(), ds([[1, 2], [3]]))

  def test_schema_as_expr(self):
    res = expr_eval.eval(
        kde.slices.slice(I.data, I.schema),  # pyrefly: ignore[missing-attribute]
        data=ds([1, 2, 3]),
    ...
    res = expr_eval.eval(
        kde.slices.slice([[], []], I.schema), schema=schema_constants.INT64  # pyrefly: ignore[missing-attribute]
    )
    testing.assert_equal(res, ds([[], []], schema_constants.INT64))

    res = expr_eval.eval(
        kde.slices.slice(ds([1, 2, 3]), I.schema), schema=schema_constants.INT64  # pyrefly: ignore[missing-attribute]
    )
    testing.assert_equal(res, ds([1, 2, 3], schema_constants.INT64))

    res = expr_eval.eval(
        kde.slices.slice([ds(1), ds(2), ds(3)], I.schema),  # pyrefly: ignore[missing-attribute]
        schema=schema_constants.INT64,
    )
    testing.assert_equal(res, ds([1, 2, 3], schema_constants.INT64))
    ...
    with self.assertRaisesRegex(
      ...
      _ = kde.slices.slice([1, 2, 3], I.schema)  # pyrefly: ignore[missing-attribute]
  ...
  def test_schema_adoption(self):
    ...
    res = expr_eval.eval(kde.slices.slice([], schema))  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(res.get_schema().a.no_bag(), schema_constants.INT32)
    res = expr_eval.eval(kde.slices.slice([ds(None)], schema))  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(res.get_schema().a.no_bag(), schema_constants.INT32)
    res = expr_eval.eval(kde.slices.slice([I.x], schema), x=None)  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(res.get_schema().a.no_bag(), schema_constants.INT32)
    res = expr_eval.eval(kde.slices.slice(I.x, I.schema), schema=schema, x=None)  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(res.get_schema().a.no_bag(), schema_constants.INT32)
    res = expr_eval.eval(kde.slices.slice([], I.schema), schema=schema)  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(res.get_schema().a.no_bag(), schema_constants.INT32)
    res = expr_eval.eval(kde.slices.slice([ds(None)], I.schema), schema=schema)  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(res.get_schema().a.no_bag(), schema_constants.INT32)

  def test_does_not_go_through_float32(self):
    x = [1 + 1e-14, 2]
    testing.assert_equal(
        expr_eval.eval(kde.slices.slice(x, schema_constants.FLOAT64)),  # pyrefly: ignore[missing-attribute]
        ds(x, schema_constants.FLOAT64),
    )

  def test_boxing(self):
    testing.assert_equal(
        kde.slices.slice([1, 2, 3]),  # pyrefly: ignore[missing-attribute]
        arolla.abc.bind_op(
            kde.slices.slice,  # pyrefly: ignore[missing-attribute]
    ...
    testing.assert_equal(
        kde.slices.slice([1, 2, 3], schema_constants.INT64),  # pyrefly: ignore[missing-attribute]
        arolla.abc.bind_op(
            kde.slices.slice,  # pyrefly: ignore[missing-attribute]
    ...
    testing.assert_equal(
        kde.slices.slice(ds([1, 2, 3]), schema_constants.INT64),  # pyrefly: ignore[missing-attribute]
        arolla.abc.bind_op(
            kde.slices.slice,  # pyrefly: ignore[missing-attribute]
            literal_operator.literal(ds([1, 2, 3])),
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.slices.slice([1, 2, 3])))  # pyrefly: ignore[missing-attribute]

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.slices.slice, kde.slice))  # pyrefly: ignore[missing-attribute]
