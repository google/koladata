...
class KodaInternalParallelStreamMakeTest(absltest.TestCase):
  ...
  def test_make(self):
    res = kd_internal.parallel.stream_make(1, 2)
  ...
  def test_make_with_bags(self):
    ...
    res = kd_internal.parallel.stream_make(db1, db2)
    self.assertIsInstance(res, clib.Stream)
  ...
  def test_make_with_bags_explicit_value_type_as(self):
    db1 = data_bag.DataBag.empty_mutable()
    db2 = data_bag.DataBag.empty_mutable()
    res = kd_internal.parallel.stream_make(
  ...
  def test_make_with_bags_wrong_value_type_as(self):
    ...
    with self.assertRaisesRegex(
      ...
      _ = kd_internal.parallel.stream_make(db1, db2, value_type_as=ds(1))

  def test_make_mixed_types(self):
    with self.assertRaisesRegex(
        ValueError, re.escape('all items must have the same type')
    ):
      _ = kd_internal.parallel.stream_make(1, data_bag.DataBag.empty_mutable())
  ...
  def test_make_empty_with_value_type_as(self):
    res = kd_internal.parallel.stream_make(value_type_as=data_bag.DataBag)
    self.assertIsInstance(res, clib.Stream)
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde_internal.parallel.stream_make()))  # pyrefly: ignore[missing-attribute]

  def test_repr(self):
    self.assertEqual(
        repr(kde_internal.parallel.stream_make(I.a, I.b)),  # pyrefly: ignore[missing-attribute]
