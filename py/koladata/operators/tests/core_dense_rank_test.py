...
class CoreOrdinalRankTest(parameterized.TestCase):
  ...
  def test_eval(self, x, descending, ndim, expected):
    result = expr_eval.eval(
        kde.core.dense_rank(x, descending=descending, ndim=ndim)
    )
  ...
  def test_eval_with_descending_ndim_unspecified(self, x, expected):
    result = expr_eval.eval(kde.core.dense_rank(x))
    testing.assert_equal(result, expected)
  ...
  def test_out_of_bounds_ndim_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      expr_eval.eval(kde.core.dense_rank(x, ndim=-1))
    with self.assertRaisesRegex(
        ValueError, re.escape('expected 0 <= ndim <= rank')
    ):
      expr_eval.eval(kde.core.dense_rank(x, ndim=2))
