...
class SchemaToInt64Test(parameterized.TestCase):
  ...
  def test_not_castable_error(self, value):
    with self.assertRaisesRegex(
        ValueError,
        f"casting a DataSlice with schema {value.get_schema()} to INT64 is not"
        " supported",
    ):
      kd.schema.to_int64(value)
  ...
  def test_not_parseable_error(self):
    with self.assertRaisesRegex(
      ...
      kd.schema.to_int64(ds("1.5"))

  def test_boxing(self):
    testing.assert_equal(
        kde.schema.to_int64(1),  # pyrefly: ignore[missing-attribute]
        arolla.abc.bind_op(
            kde.schema.to_int64, literal_operator.literal(ds(1))  # pyrefly: ignore[missing-attribute]
        ),
    )
  ...
  def test_qtype_signatures(self):
    self.assertCountEqual(
        arolla.testing.detect_qtype_signatures(
            kde.schema.to_int64,  # pyrefly: ignore[missing-attribute]
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.schema.to_int64(I.x)))  # pyrefly: ignore[missing-attribute]
