...
class LogicalHasNotTest(parameterized.TestCase):
  ...
  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        kde.masking.has_not,  # pyrefly: ignore[missing-attribute]
        QTYPES,
        possible_qtypes=qtypes.DETECT_SIGNATURES_QTYPES,  # pyrefly: ignore[bad-argument-type]
    )

  def test_repr(self):
    self.assertEqual(repr(kde.masking.has_not(I.x)), '~I.x')  # pyrefly: ignore[missing-attribute]
    self.assertEqual(repr(kde.has_not(I.x)), '~I.x')  # pyrefly: ignore[missing-attribute]

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.masking.has_not(I.x)))  # pyrefly: ignore[missing-attribute]

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.masking.has_not, kde.has_not))  # pyrefly: ignore[missing-attribute]
