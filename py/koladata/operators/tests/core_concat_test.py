...
class CoreConcatImplTest(parameterized.TestCase):
  ...
  def test_eval(self, args, ndim, expected):
    result = expr_eval.eval(kde.core.concat(*args, ndim=ndim))
  ...
  def test_invalid_rank_or_ndim(self, args, ndim, expected_regex):
    with self.assertRaisesRegex(ValueError, re.escape(expected_regex)):
      expr_eval.eval(kde.core.concat(*args, ndim=ndim))

  def test_same_databag(self):
    db = data_bag.DataBag.empty()
    a = ds([db.obj(x=1)])
    b = ds([db.obj(x=2)])
    result = expr_eval.eval(kde.core.concat(a, b))
  ...
  def test_multiple_databag(self):
    ...
    b = db2.obj(x=2)
    b_slice = ds([b])
    result = expr_eval.eval(kde.core.concat(a_slice, b_slice))
    ...
    testing.assert_equal(result, ds([a.no_db(), b.no_db()]).with_db(result.db))

  def test_qtype_signatures(self):
    signature = arolla.abc.get_operator_signature(kde.core.concat)
  ...
  def test_view(self):
    self.assertTrue(view.has_data_slice_view(kde.core.concat(I.x, I.y, I.z)))
