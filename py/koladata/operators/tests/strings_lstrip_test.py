...
class StringsLstripTest(parameterized.TestCase):
  ...
  def test_two_args(self, s, chars, expected):
    result = kd.strings.lstrip(s, chars)
    testing.assert_equal(result, expected)
  ...
  def test_incompatible_types(self):
    with self.assertRaisesRegex(
      ...
      kd.strings.lstrip(ds('foo'), ds(b'f'))
    ...
    with self.assertRaisesRegex(
      ...
      kd.strings.lstrip(ds([None], schema_constants.STRING), ds(123))
  ...
  def test_mixed_slice_error(self):
    with self.assertRaisesRegex(
      ...
      kd.strings.lstrip(ds('foo'), ds(['fo', 123]))
