...
class CoreAggSizeTest(parameterized.TestCase):
  ...
  @parameterized.parameters(
      (
          ...
          ds([3, 2, 2], schema_constants.INT64),
      ),
      (
          ds([[1, None, None], [3.0, 4], [None, None]]),
          arolla.unspecified(),
      ...
      (
          ds([[1, None, None], [3.0, 4], [None, None]]),
          ds(0),
          ds([[1, 1, 1], [1, 1], [1, 1]], schema_constants.INT64),
      ),
      (ds([1, None, 'b']), ds(3, schema_constants.INT64)),
      (ds([None], schema_constants.INT64), ds(1, schema_constants.INT64)),
      (ds(['a', None]), ds(2, schema_constants.INT64)),
      (
          ds([[[1, None], [3, 4]], [[None, None]]]),
          ds([[2, 2], [2]], schema_constants.INT64),
      ),
  )
  def test_eval(self, *args_and_expected):
    args, expected_value = args_and_expected[:-1], args_and_expected[-1]
    result = expr_eval.eval(kde.core.agg_size(*args))
  ...
  def test_data_item_input_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      expr_eval.eval(kde.core.agg_size(x))
  ...
  def test_out_of_bounds_ndim_error(self, ndim):
    ...
    with self.assertRaisesRegex(ValueError, 'expected 0 <= ndim <= rank'):
      expr_eval.eval(kde.core.agg_size(x, ndim))
  ...
  def test_view(self):
    self.assertTrue(view.has_data_slice_view(kde.core.agg_size(I.x)))

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.core.agg_size, kde.agg_size))
