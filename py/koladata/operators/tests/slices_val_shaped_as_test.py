...
class SlicesValShapedAsTest(parameterized.TestCase):
  ...
  def test_incompatible_shape(self):
    ...
    with self.assertRaisesRegex(ValueError, re.escape('cannot be expanded')):
      _ = kd.slices.val_shaped_as(ds([['1'], [None, '3']]), ds([1, 2, 3]))

  def test_qtype_signatures(self):
    self.assertCountEqual(
        arolla.testing.detect_qtype_signatures(
            kde.slices.val_shaped_as,  # pyrefly: ignore[missing-attribute]
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.slices.val_shaped_as(I.x, I.val)))  # pyrefly: ignore[missing-attribute]
