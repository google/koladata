...
class JsonStreamSelectNonemptyObjectsTest(parameterized.TestCase):

  def test_basic(self):
    result = kd.json_stream.select_nonempty_objects(
        kd.iterables.make('{}\n', '{"x', '":1}\n{}\n2', '\n')
    )
  ...
  def test_parallel_transform(self):
    ...
    result = kd_internal.parallel.transform(
        ...
        executor,
        x=kd.streams.make('{"x":1}\n{}\n2\n'),
        return_type_as=kd.streams.make(),
    ).yield_all(
        timeout=1
    )
    testing.assert_equal(kd.stack(*result), ds(['{"x":1}\n']))
  ...
  def test_bad_arguments(self):
    with self.assertRaisesRegex(
      ...
      kd.json_stream.select_nonempty_objects(kd.iterables.make(123))
    with self.assertRaisesRegex(
        ValueError,
        re.escape(
            'expected STRING DataItem from input stream, got slice with schema'
      ...
      kd.json_stream.select_nonempty_objects(kd.iterables.make(ds(['x', 'y'])))
