...
class SlicesGetReprTest(parameterized.TestCase):
  ...
  def test_eval(self, x, expected):
    res = kd.slices.get_repr(x)
    testing.assert_equal(res, expected)

  def test_depth(self):
    res = kd.slices.get_repr(db.obj(a=db.obj(a=1), b='foo'), depth=1)
  ...
  def test_item_limit(self):
    res = kd.slices.get_repr(ds([1, 2]), item_limit=1)
  ...
  def test_schema_item_limit(self):
    schema = db.new(a=1, b=2, c=3, d=4).get_schema()
    res = kd.slices.get_repr(schema, item_limit=2)
    self.assertEqual(res.to_py(), 'ENTITY(a=INT32, b=INT32, ...)')
  ...
  def test_format_html(self):
    res = kd.slices.get_repr(ds(['foo', 'bar']), format_html=True)
  ...
  def test_show_attributes(self):
    res = kd.slices.get_repr(db.obj(a=ds([1, 2])), show_attributes=False)
    self.assertRegex(res.to_py(), r'^\[Obj:\$.{22}, Obj:\$.{22}\]$')

  def test_show_databag_id(self):
    res = kd.slices.get_repr(db.obj(a=ds([1, 2])), show_databag_id=True)
    self.assertRegex(
        res.to_py(),
  ...
  def test_show_present_count(self):
    res = kd.slices.get_repr(ds([[1, 2], [None, 4]]), show_present_count=True)
    testing.assert_equal(
  ...
  def test_print_only_attr_names(self):
    res = kd.slices.get_repr(
        db.new(a=ds([1, 2])), item_limit=1, show_attributes=True
    )
  ...
  def test_eval_like(self, x, expected_regex):
    res = kd.slices.get_repr(x).internal_as_py()
