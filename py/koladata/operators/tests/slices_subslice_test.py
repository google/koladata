...
class SlicesSubsliceTest(parameterized.TestCase):
  ...
  @parameterized.parameters(
      ...
      (
          ds([[1, 2], [3], [4, 5, 6]]),
          [slice(3, 5), slice(1, 5)],
          ds([], schema_constants.INT32).reshape(kd.shapes.new(0, [])),
      ),
      # Repeated indices
      (
          ds([[1, 2], [3], [4, 5, 6]]),
  ...
  def test_stress_against_list_ops(self):
    ...
    for seed in range(0, num_tests * 1000, 1000):
      ...
      for step in range(num_dims):
        data = data.repeat(
            kd.randint_like(data, min_size, max_size + 1, seed=seed + step)
        )
      # Replace just "present" with random values.
      data = kd.randint_like(data, 100, seed=seed + 10)
      # Choose random number of dimensions to slice.
      dims_to_slice = kd.randint_like(present, 1, num_dims + 1, seed=seed + 20)
      indices = []
      chosen = kd.implode(data, dims_to_slice)
      for step in range(dims_to_slice):
        mode = kd.randint_like(present, 5, seed=seed + 30 + step)
        if mode == 0:
          ...
          num_to_take = (
              kd.randint_like(chosen, max_take + 1, seed=seed + 40 + step) | 0
          )
          # Choose random indices to take.
          to_take = kd.randint_like(
  ...
  def test_invalid_qtype_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      kd.subslice(ds([1, 2, 3]), slice('a', 3))
    ...
    with self.assertRaisesRegex(
      ...
      kd.subslice(ds([1, 2, 3]), slice(3, 'a'))
    ...
    with self.assertRaisesRegex(
      ...
      kd.subslice(ds([1, 2, 3]), slice(3, ds('1')))
    ...
    with self.assertRaisesRegex(
      ...
      kd.subslice(ds([1, 2, 3]), slice(3, ds(1.0)))
    ...
    with self.assertRaisesRegex(
      ...
      kd.subslice(ds([1, 2, 3]), slice(3, 4, 2))
  ...
  def test_invalid_number_of_slices_error(self):
    with self.assertRaisesRegex(
      ...
      kd.subslice(ds([1, 2, 3]), slice(1), slice(1))
