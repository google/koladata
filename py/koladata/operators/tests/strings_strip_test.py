...
class StringsStripTest(parameterized.TestCase):
  ...
  def test_two_args(self, s, chars, expected):
    result = kd.strings.strip(s, chars)
    testing.assert_equal(result, expected)
  ...
  def test_incompatible_types(self):
    with self.assertRaisesRegex(
        ValueError,
        re.escape(
            'kd.strings.strip: mixing STRING and BYTES arguments is not'
            ' allowed, but `s` contains STRING and `chars` contains BYTES'
        ),
    ):
      kd.strings.strip(ds('foo'), ds(b'f'))

    with self.assertRaisesRegex(
        ValueError,
        re.escape(
            'kd.strings.strip: argument `chars` must be a slice of either'
            ' STRING or BYTES, got a slice of INT32'
        ),
    ):

      kd.strings.strip(ds([None], schema_constants.STRING), ds(123))
    ...
    with self.assertRaisesRegex(
      ...
      kd.strings.strip(None, 123)

  def test_mixed_slice_error(self):
    with self.assertRaisesRegex(
        ValueError,
        'kd.strings.strip: argument `chars` must be a slice of either STRING or'
        ' BYTES, got a slice of OBJECT containing INT32 and STRING values',
    ):
      kd.strings.strip(ds('foo'), ds(['fo', 123]))

  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        kde.strings.strip,  # pyrefly: ignore[missing-attribute]
        QTYPES,
        possible_qtypes=test_qtypes.DETECT_SIGNATURES_QTYPES,  # pyrefly: ignore[bad-argument-type]
    )

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.strings.strip(I.a)))  # pyrefly: ignore[missing-attribute]
