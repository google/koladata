...
class KodaInternalParallelFutureIterableFromStreamTest(absltest.TestCase):
  ...
  def test_simple(self):
    ...
    expr = kde_internal.parallel.future_iterable_from_stream(stream)
  ...
  def test_qtype_signatures(self):
    ...
    future_iterable_stream_int32_qtype = expr_eval.eval(
        kde_internal.parallel.get_future_qtype(
            kde_internal.iterables.get_iterable_qtype(stream_int32_qtype)
        )
    )
    arolla.testing.assert_qtype_signatures(
        kde_internal.parallel.future_iterable_from_stream,
        [
  ...
  def test_view(self):
    self.assertTrue(
        view.has_koda_view(
            kde_internal.parallel.future_iterable_from_stream(I.x)
        )
    )
