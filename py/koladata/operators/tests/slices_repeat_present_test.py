...
class SlicesRepeatPresentTest(parameterized.TestCase):
  ...
  def test_eval(self, args, size_schema):
    ...
    actual_value = kd.slices.repeat_present(x, size)
  ...
  def test_zero_repeat_present(self):
    ...
    res = kd.slices.repeat_present(ds([[1, 2], [3]]), ds(0))
  ...
  def test_align_arguments(self):
    testing.assert_equal(
        kd.slices.repeat_present(ds(1), ds([1, 2, 3])),
        ds([[1], [1, 1], [1, 1, 1]]),
    )
