...
class SlicesRepeatPresentTest(parameterized.TestCase):
  ...
  def test_eval(self, args, size_schema):
    ...
    actual_value = expr_eval.eval(kde.slices.repeat_present(x, size))
  ...
  def test_zero_repeat_present(self):
    ...
    res = expr_eval.eval(kde.slices.repeat_present(ds([[1, 2], [3]]), ds(0)))
    print(res)
  ...
  @parameterized.parameters(
      kde.repeat_present, kde.slices.add_dim_to_present, kde.add_dim_to_present
  )
  def test_alias(self, alias):
    self.assertTrue(optools.equiv_to_op(kde.slices.repeat_present, alias))
