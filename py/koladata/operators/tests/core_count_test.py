...
class CoreCountTest(parameterized.TestCase):
  ...
  def test_eval(self, *args_and_expected):
    ...
    result = expr_eval.eval(kde.core.count(*args))
  ...
  def test_view(self):
    self.assertTrue(view.has_data_slice_view(kde.core.count(I.x)))

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.core.count, kde.count))
