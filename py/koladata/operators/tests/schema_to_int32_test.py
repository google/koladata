...
class SchemaToInt32Test(parameterized.TestCase):
  ...
  def test_boxing(self):
    testing.assert_equal(
        kde.schema.to_int32(1),  # pyrefly: ignore[missing-attribute]
        arolla.abc.bind_op(
            kde.schema.to_int32, literal_operator.literal(ds(1))  # pyrefly: ignore[missing-attribute]
        ),
    )
  ...
  def test_qtype_signatures(self):
    self.assertCountEqual(
        arolla.testing.detect_qtype_signatures(
            kde.schema.to_int32,  # pyrefly: ignore[missing-attribute]
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.schema.to_int32(I.x)))  # pyrefly: ignore[missing-attribute]
