...
class SchemaToInt32Test(parameterized.TestCase):
  ...
  def test_not_castable_internal_value(self):
    ...
    with self.assertRaisesRegex(ValueError, "cannot cast TEXT to INT32"):
      expr_eval.eval(kde.schema.to_int32(x))

  def test_boxing(self):
    testing.assert_equal(
        kde.schema.to_int32(1),
        arolla.abc.bind_op(
            kde.schema.to_int32, literal_operator.literal(ds(1))
        ),
    )
  ...
  def test_view(self):
    self.assertTrue(view.has_data_slice_view(kde.schema.to_int32(I.x)))

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.schema.to_int32, kde.to_int32))
