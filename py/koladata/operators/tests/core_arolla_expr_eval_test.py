...
class CoreArollaExprEvalTest(absltest.TestCase):

  def test_simple_expr(self):
    res = kd.core._arolla_expr_eval(
  ...
  def test_errors(self):
    with self.assertRaisesRegex(
      ...
      kd.core._arolla_expr_eval(ds(0), ds(1))
    with self.assertRaisesRegex(
        ValueError, 'first argument is expected to be a DataItem with ExprQuote'
    ):
      kd.core._arolla_expr_eval(ds(0), kd.tuples.namedtuple())
    with self.assertRaisesRegex(
