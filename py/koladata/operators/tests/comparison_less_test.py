...
class ComparisonLessTest(parameterized.TestCase):
  ...
  def test_qtype_difference(self):
    ...
    with self.assertRaisesRegex(
        ValueError,
        re.escape(
            r"""kd.comparison.less: arguments do not contain values castable to a common primitive schema, but have the common non-primitive schema OBJECT.
...
Schema for `y`: STRING"""
  ...
  def test_qtype_signatures(self):
    self.assertCountEqual(
        arolla.testing.detect_qtype_signatures(
            kde.comparison.less,  # pyrefly: ignore[missing-attribute]
  ...
  def test_repr(self):
    self.assertEqual(repr(kde.comparison.less(I.x, I.y)), 'I.x < I.y')  # pyrefly: ignore[missing-attribute]
    self.assertEqual(repr(kde.less(I.x, I.y)), 'I.x < I.y')  # pyrefly: ignore[missing-attribute]

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.comparison.less(I.x, I.y)))  # pyrefly: ignore[missing-attribute]

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.comparison.less, kde.less))  # pyrefly: ignore[missing-attribute]
