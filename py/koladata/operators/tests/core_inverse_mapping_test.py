...
"""Tests for core.inverse_mapping."""
...
class CoreInverseMappingTest(parameterized.TestCase):
  ...
  def test_eval(self, x, ndim, expected):
    result = expr_eval.eval(kde.core.inverse_mapping(x, ndim=ndim))
  ...
  def test_eval_with_ndim_unspecified(self, x, expected):
    result = expr_eval.eval(kde.core.inverse_mapping(x))
    testing.assert_equal(result, expected)
  ...
  def test_out_of_bounds_ndim_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      expr_eval.eval(kde.core.inverse_mapping(x, ndim=-1))
    with self.assertRaisesRegex(
        ValueError, re.escape('expected 0 <= ndim <= rank')
    ):
  ...
  def test_view(self):
    self.assertTrue(view.has_data_slice_view(kde.core.inverse_mapping(I.x)))

  def test_alias(self):
    self.assertTrue(
        optools.equiv_to_op(kde.core.inverse_mapping, kde.inverse_mapping)
    )
