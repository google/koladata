...
class KodaInternalParallelStreamFlatMapInterleavedTest(absltest.TestCase):

  def test_basic(self):
    fn = expr_fn(
        koda_internal_parallel.stream_make(I.self, 2 * I.self, 3 * I.self)
    )
    res = koda_internal_parallel.stream_flat_map_interleaved(
    ...
    self.assertCountEqual(
        res_list, [ds(1), ds(2), ds(3), ds(10), ds(20), ds(30)]
    )
  ...
  def test_with_value_type_as(self):
    ...
    res_list = res.read_all(timeout=None)
    self.assertCountEqual(res_list, [1, 3, 5, 10, 15, 30])
    self.assertContainsSubsequence(res_list, [1, 3])
    self.assertContainsSubsequence(res_list, [5, 15])
    self.assertContainsSubsequence(res_list, [10, 30])
  ...
  def test_view(self):
    self.assertTrue(
        view.has_koda_view(
            koda_internal_parallel.stream_flat_map_interleaved(
  ...
  def test_repr(self):
    self.assertEqual(
        ...
        'koda_internal.parallel.stream_flat_map_interleaved(I.executor,'
