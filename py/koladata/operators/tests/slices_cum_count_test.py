...
class SlicesCumCountTest(parameterized.TestCase):
  ...
  def test_eval(self, x, ndim, expected):
    result = expr_eval.eval(kde.slices.cum_count(x, ndim))
  ...
  def test_out_of_bounds_ndim_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      expr_eval.eval(kde.slices.cum_count(x, -1))
    with self.assertRaisesRegex(
        ValueError, re.escape('expected 0 <= ndim <= rank')
    ):
      expr_eval.eval(kde.slices.cum_count(x, 2))
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.slices.cum_count(I.x)))

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.slices.cum_count, kde.cum_count))
