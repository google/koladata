...
class SlicesCumCountTest(parameterized.TestCase):
  ...
  def test_eval(self, x, ndim, expected):
    result = kd.slices.cum_count(x, ndim)
  ...
  def test_out_of_bounds_ndim_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      kd.slices.cum_count(x, -1)
    with self.assertRaisesRegex(
        ValueError, re.escape('expected 0 <= ndim <= rank')
    ):
      kd.slices.cum_count(x, 2)

  def test_qtype_signatures(self):
    self.assertCountEqual(
        arolla.testing.detect_qtype_signatures(
            kde.slices.cum_count,  # pyrefly: ignore[missing-attribute]
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.slices.cum_count(I.x)))  # pyrefly: ignore[missing-attribute]

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.slices.cum_count, kde.cum_count))  # pyrefly: ignore[missing-attribute]
