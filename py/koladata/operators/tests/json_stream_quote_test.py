...
class JsonStreamQuoteTest(parameterized.TestCase):

  def test_basic(self):
    result = kd.json_stream.quote(kd.iterables.make('hello world'))
    testing.assert_equal(result, kd.iterables.make('"hello world', '"'))

  def test_parallel_transform(self):
    executor = kd_internal.parallel.get_default_executor()
    result = parallel_fns.transform(
        functor_factories.expr_fn(returns=kde.json_stream.quote(I.x)),  # pyrefly: ignore[missing-attribute]
    )(
        executor,
        x=kd.streams.make('hello world'),
        return_type_as=kd.streams.make(),
    ).yield_all(
        timeout=1
    )
    testing.assert_equal(kd.stack(*result), ds(['"hello world', '"']))
  ...
  def test_bad_arguments(self):
    with self.assertRaisesRegex(
      ...
      kd.json_stream.quote(kd.iterables.make(123))
    with self.assertRaisesRegex(
        ValueError,
        re.escape(
            'expected STRING DataItem from input stream, got slice with schema'
      ...
      kd.json_stream.quote(kd.iterables.make(ds(['x', 'y'])))
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.json_stream.quote(I.x)))  # pyrefly: ignore[missing-attribute]
