...
class FunctorBindTest(absltest.TestCase):
  ...
  def test_no_params(self):
    ...
    bound_fn = kde.functor.bind(fn).eval()  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(bound_fn(x=5, y=7), ds(12))

  def test_full_params(self):
    fn = kde.functor.expr_fn(ds(arolla.quote(I.x + I.y)))  # pyrefly: ignore[missing-attribute, unsupported-operation]
    bound_fn = kde.functor.bind(fn, x=5, y=7).eval()  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(bound_fn(), ds(12))

  def test_bind_partial_params(self):
    fn = kde.functor.expr_fn(ds(arolla.quote(I.x + I.y)))  # pyrefly: ignore[missing-attribute, unsupported-operation]
    bound_fn = kde.functor.bind(fn, x=5).eval()  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(bound_fn(y=7), ds(12))

  def test_bind_returns_databag(self):
    fn = functor_factories.expr_fn(I.x.get_bag())
    x = fns.obj(q=1)
    bound_fn = kde.functor.bind(fn, return_type_as=data_bag.DataBag).eval()  # pyrefly: ignore[missing-attribute]
  ...
  def test_fn_is_not_ds(self):
    with self.assertRaisesRegex(
      ...
      kde.functor.bind(arolla.int64(57))  # pyrefly: ignore[missing-attribute]

  def test_fn_is_wrong_functor(self):
    with self.assertRaisesRegex(ValueError, 'fn must be a data item'):
      kde.functor.bind(ds([]), x=5).eval()  # pyrefly: ignore[missing-attribute]
    with self.assertRaisesRegex(ValueError, 'fn must be present'):
      kde.functor.bind(ds(None), x=5).eval()  # pyrefly: ignore[missing-attribute]
    with self.assertRaisesRegex(ValueError, 'must be a functor'):
      kde.functor.bind(ds(57), x=5).eval()()  # pyrefly: ignore[missing-attribute]
  ...
  def test_bound_rank_1(self):
    ...
    with self.assertRaisesRegex(ValueError, 'variable.*x.*must be a data item'):
      kde.functor.bind(fn, x=ds([5, 9])).eval()  # pyrefly: ignore[missing-attribute]
  ...
  def test_bind_positional_or_keyword(self):
    ...
    fn = kde.functor.expr_fn(ds(arolla.quote(I.x + I.y)), signature=signature)  # pyrefly: ignore[missing-attribute, unsupported-operation]
    bound_fn = kde.functor.bind(fn, x=5).eval()  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(bound_fn(y=7), ds(12))
  ...
  def test_bind_positional_args(self):
    ...
    fn = kde.functor.expr_fn(ds(arolla.quote(I.x + I.y)), signature=signature)  # pyrefly: ignore[missing-attribute, unsupported-operation]
    bound_fn = kde.functor.bind(fn, 5).eval()  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(bound_fn(7), ds(12))
    bound_fn_2 = kde.functor.bind(fn, 5, 7).eval()  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(bound_fn_2(), ds(12))
    bound_fn_3 = kde.functor.bind(fn, 5, y=7).eval()  # pyrefly: ignore[missing-attribute]
    testing.assert_equal(bound_fn_3(), ds(12))

  def test_bound_data_bag_conflict(self):
    x = kd.new(x=1)
    y = x.with_attrs(x=2)
    fn = kde.functor.expr_fn(ds(arolla.quote(I.x.x + I.y.x)))  # pyrefly: ignore[missing-attribute]
    with self.assertRaisesRegex(ValueError, 'conflict'):
      kde.functor.bind(fn, x=x, y=y).eval()  # pyrefly: ignore[missing-attribute]

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.functor.bind(I.fn)))  # pyrefly: ignore[missing-attribute]

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.functor.bind, kde.bind))  # pyrefly: ignore[missing-attribute]
