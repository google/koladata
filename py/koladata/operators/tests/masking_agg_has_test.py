...
class LogicalAggHasTest(parameterized.TestCase):
  ...
  def test_eval(self, *args_and_expected):
    ...
    result = kd.masking.agg_has(*args)
  ...
  def test_data_item_input_error(self):
    ...
    with self.assertRaisesRegex(ValueError, re.escape('expected rank(x) > 0')):
      kd.masking.agg_has(x)

  @parameterized.parameters(-1, 2)
  def test_out_of_bounds_ndim_error(self, ndim):
    x = ds([1, 2, 3])
    with self.assertRaisesRegex(ValueError, 'expected 0 <= ndim <= rank'):
      kd.masking.agg_has(x, ndim=ndim)

  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        kde.masking.agg_has,  # pyrefly: ignore[missing-attribute]
        QTYPES,
        possible_qtypes=qtypes.DETECT_SIGNATURES_QTYPES,  # pyrefly: ignore[bad-argument-type]
    )

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.masking.agg_has(I.x)))  # pyrefly: ignore[missing-attribute]

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.masking.agg_has, kde.agg_has))  # pyrefly: ignore[missing-attribute]
