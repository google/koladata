...
class JsonStreamChunkValuesTest(parameterized.TestCase):

  def test_basic(self):
    result = kd.json_stream.chunk_values(
        kd.iterables.make('1\n[2,', '3]\n{"x":4}\n')
    )
    testing.assert_equal(
        result, kd.iterables.make('1\n', '[2,3]\n', '{"x":4}\n')
    )

  def test_empty(self):
    result = kd.json_stream.chunk_values(kd.iterables.make(''))
    testing.assert_equal(result, kd.iterables.make())

  def test_single_value(self):
    result = kd.json_stream.chunk_values(kd.iterables.make('null\n'))
    testing.assert_equal(result, kd.iterables.make('null\n'))

  def test_compactifies_whitespace(self):
    result = kd.json_stream.chunk_values(kd.iterables.make('  { "x" : 1 }  \n'))
    testing.assert_equal(result, kd.iterables.make('{"x":1}\n'))

  def test_parallel_transform(self):
    executor = kd_internal.parallel.get_default_executor()
    result = parallel_fns.transform(
        functor_factories.expr_fn(returns=kde.json_stream.chunk_values(I.x)),  # pyrefly: ignore[missing-attribute]
    )(
        executor,
        x=kd.streams.make('1\n2\n3\n'),
  ...
  def test_bad_arguments(self):
    with self.assertRaisesRegex(
      ...
      kd.json_stream.chunk_values(kd.iterables.make(123))
    with self.assertRaisesRegex(
        ValueError,
        re.escape(
            'expected STRING DataItem from input stream, got slice with schema'
      ...
      kd.json_stream.chunk_values(kd.iterables.make(ds(['x', 'y'])))
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.json_stream.chunk_values(I.x)))  # pyrefly: ignore[missing-attribute]
