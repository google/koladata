...
class SchemaInternalCastToNarrowTest(parameterized.TestCase):
  ...
  def test_eval(self, x, schema, expected):
    res = kd.schema.internal_cast_to_narrow(x, schema)
  ...
  def test_explicit_entity_schema(self):
    ...
    res = kd.schema.internal_cast_to_narrow(obj, entity.get_schema())
  ...
  def test_implicit_entity_schema_error(self):
    ...
    with self.assertRaisesRegex(
      ...
      kd.schema.internal_cast_to_narrow(obj, obj.get_obj_schema())
  ...
  def test_not_schema_error(self):
    with self.assertRaisesRegex(
      ...
      kd.schema.internal_cast_to_narrow(ds(1), ds(1))
 ...
 the first conflicting schema ITEMID"""),
  ...
  def test_boxing(self):
    testing.assert_equal(
        kde.schema.internal_cast_to_narrow(1, schema_constants.INT64),
        arolla.abc.bind_op(
            kde.schema.internal_cast_to_narrow,
  ...
  def test_view(self):
    self.assertTrue(
        view.has_koda_view(kde.schema.internal_cast_to_narrow(I.x, I.y))
    )
