...
class MathCeilTest(parameterized.TestCase):
  ...
  def test_eval_numeric(self, x, expected):
    result = expr_eval.eval(kde.math.ceil(I.x), x=x)
  ...
  def test_errors(self):
    ...
    with self.assertRaisesRegex(
        ...
        re.escape(
            'argument `x` must be a slice of numeric values, got a slice of'
            ' STRING'
        ),
    ):
      expr_eval.eval(kde.math.ceil(I.x), x=x)
