...
class JsonStreamExplodeArrayTest(parameterized.TestCase):

  def test_basic(self):
    result = kd.json_stream.explode_array(kd.iterables.make('[1,2] 3 [4,[5]]'))
  ...
  def test_parallel_transform(self):
    ...
    result = parallel_fns.transform(
        functor_factories.expr_fn(returns=kde.json_stream.explode_array(I.x)),  # pyrefly: ignore[missing-attribute]
    )(
        executor,
        x=kd.streams.make('[1,2] 3 [4,[5]]'),
  ...
  def test_bad_arguments(self):
    with self.assertRaisesRegex(
      ...
      kd.json_stream.explode_array(kd.iterables.make(123))
    with self.assertRaisesRegex(
        ValueError,
        re.escape(
            'expected STRING DataItem from input stream, got slice with schema'
      ...
      kd.json_stream.explode_array(kd.iterables.make(ds(['x', 'y'])))
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.json_stream.explode_array(I.x)))  # pyrefly: ignore[missing-attribute]
