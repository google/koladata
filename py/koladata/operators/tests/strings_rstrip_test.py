...
class StringsRstripTest(parameterized.TestCase):
  ...
  def test_incompatible_types(self):
    with self.assertRaisesRegex(
        ValueError,
        re.escape(
            'kd.strings.rstrip: mixing STRING and BYTES arguments is not'
    ...
    with self.assertRaisesRegex(
        ValueError,
        re.escape(
            'kd.strings.rstrip: argument `chars` must be a slice of either'
  ...
  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        kde.strings.rstrip,  # pyrefly: ignore[missing-attribute]
        QTYPES,
        possible_qtypes=test_qtypes.DETECT_SIGNATURES_QTYPES,  # pyrefly: ignore[bad-argument-type]
    )

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.strings.rstrip(I.a)))  # pyrefly: ignore[missing-attribute]
