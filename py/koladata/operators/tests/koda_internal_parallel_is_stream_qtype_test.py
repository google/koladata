...
class KodaInternalParallelIsStreamQTypeTest(parameterized.TestCase):
  ...
  def testFalse(self, value_qtype):
    testing.assert_equal(
        arolla.eval(koda_internal_parallel.is_stream_qtype(value_qtype)),
        arolla.missing_unit(),
    )
  ...
  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        koda_internal_parallel.is_stream_qtype,
        [(arolla.QTYPE, arolla.OPTIONAL_UNIT)],
    )

  def test_view(self):
    self.assertFalse(
        view.has_koda_view(koda_internal_parallel.is_stream_qtype(I.x))
    )
