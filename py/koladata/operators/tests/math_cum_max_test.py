...
class MathCumMaxTest(parameterized.TestCase):
  ...
  def test_repr(self):
    ...
    self.assertEqual(repr(kde.cum_max(I.x)), 'kde.cum_max(I.x, unspecified)')
    self.assertEqual(
  ...
  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.math.cum_max, kde.cum_max))
