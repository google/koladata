...
class StringsContainsTest(parameterized.TestCase):
  ...
  def test_eval(self, s, substr, expected):
    result = kd.strings.contains(s, substr)
  ...
  def test_incompatible_types_error(self):
    with self.assertRaisesRegex(
      ...
      kd.strings.contains(ds('foo'), ds(b'f'))
  ...
  def test_another_incompatible_types_error(self):
    with self.assertRaisesRegex(
      ...
      kd.strings.contains(ds([None], schema_constants.STRING), ds(123))
  ...
  def test_mixed_slice_error(self):
    with self.assertRaisesRegex(
      ...
      kd.strings.contains(ds('foo'), ds([1, 'fo']))

  def test_qtype_signatures(self):
    arolla.testing.assert_qtype_signatures(
        kde.strings.contains,  # pyrefly: ignore[missing-attribute]
        QTYPES,
        possible_qtypes=test_qtypes.DETECT_SIGNATURES_QTYPES,  # pyrefly: ignore[bad-argument-type]
    )

  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.strings.contains(I.x, I.y)))  # pyrefly: ignore[missing-attribute]
