...
class LogicalAllTest(parameterized.TestCase):
  ...
  @parameterized.parameters(
      ...
      (ds([None, 2], ds(arolla.INT32)), ds(arolla.missing())),
      (ds([[1, 2], [None, None]]), ds(arolla.missing())),
  ...
  def test_eval(self, *args_and_expected):
    ...
    result = expr_eval.eval(kde.logical.all(*args))
  ...
  def test_view(self):
    self.assertTrue(view.has_data_slice_view(kde.logical.all(I.x)))

  def test_alias(self):
    self.assertTrue(optools.equiv_to_op(kde.logical.all, kde.all))
