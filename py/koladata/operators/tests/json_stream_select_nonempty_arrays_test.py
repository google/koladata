...
class JsonStreamSelectNonemptyArraysTest(parameterized.TestCase):
  ...
  def test_parallel_transform(self):
    ...
    result = parallel_fns.transform(
        functor_factories.expr_fn(
            returns=kde.json_stream.select_nonempty_arrays(I.x)  # pyrefly: ignore[missing-attribute]
        ),
    )(
        executor,
        x=kd.streams.make('[1]\n[]\n2\n'),
  ...
  def test_view(self):
    self.assertTrue(
        view.has_koda_view(kde.json_stream.select_nonempty_arrays(I.x))  # pyrefly: ignore[missing-attribute]
    )
