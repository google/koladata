...
class SchemaCastToNarrowTest(parameterized.TestCase):
 ...
 the first conflicting schema ITEMID"""),
  ...
  def test_boxing(self):
    testing.assert_equal(
        kde.schema.cast_to_narrow(1, schema_constants.INT64),  # pyrefly: ignore[missing-attribute]
        arolla.abc.bind_op(
            kde.schema.cast_to_narrow,  # pyrefly: ignore[missing-attribute]
  ...
  def test_view(self):
    self.assertTrue(view.has_koda_view(kde.schema.cast_to_narrow(I.x, I.y)))  # pyrefly: ignore[missing-attribute]
