...
from koladata.ext.storage import global_cache_flags as _global_cache_flags
from koladata.ext.storage import lru_size_tracking_cache as _lru_size_tracking_cache

_mb_to_bytes = lambda mb: mb * 1024 * 1024


def _kd_ext_storage_global_cache_max_size_mb_validator(
    value: int,
) -> bool:
  if value < 0:
    return False
...
flags.register_validator(
    ...
    _kd_ext_storage_global_cache_max_size_mb_validator,
...
_CACHE: Cache[str, CACHE_VALUE_TYPE] = Cache(
    max_total_bytes_of_entries_in_cache=_mb_to_bytes(
        ...
        _global_cache_flags.KD_EXT_STORAGE_GLOBAL_CACHE_MAX_SIZE_MB.value
        if _global_cache_flags.KD_EXT_STORAGE_GLOBAL_CACHE_MAX_SIZE_MB.present
