...
DataBagManager = dbm.DataBagManager
...
class DataBagManagerTest(parameterized.TestCase):
  ...
  def test_typical_usage(self):
    ...
    with self.subTest('ExtractBags'):
      ...
      extracted_manager = DataBagManager.create_from_dir(new_persistence_dir)
      self.assertEqual(
  ...
  def test_use_of_provided_file_system_interaction_object(self):
    ...
    with self.subTest('EmptyInitialPersistenceDir'):
      ...
      _ = DataBagManager.create_new(persistence_dir, fs=mocked_fs)
    ...
    with self.subTest('NonEmptyInitialPersistenceDir'):
      ...
      _ = DataBagManager.create_new(persistence_dir)

      # Start a new manager with the already initialized persistence_dir.
      mocked_fs = mock.Mock(wraps=kd.file_io.FileSystemInteraction())
      _ = DataBagManager.create_from_dir(persistence_dir, fs=mocked_fs)
      method_names_called = [c[0] for c in mocked_fs.method_calls]
      self.assertEqual(
          method_names_called,
          [
    ...
    with self.subTest('extract_bags'):
      ...
      # Clear the global cache to force the loading of all bags from disk.
      global_cache_lib.get_global_cache().clear()
  ...
  def test_get_minimal_bag_with_wrong_name(self):
    persistence_dir = self.create_tempdir().full_path
    manager = DataBagManager.create_new(persistence_dir)
  ...
  def test_add_bags_in_one_go(self):
    ...
    manager.add_bags([
        ...
        dbm.BagToAdd(bag_name='bag2', bag=bag2, dependencies=('bag1',)),
        dbm.BagToAdd(bag_name='bag3', bag=bag3, dependencies=('bag1',)),
        dbm.BagToAdd(bag_name='bag4', bag=bag4, dependencies=('bag0',)),
    ])
  ...
  def test_add_bags_with_wrong_dependencies(self):
    ...
    with self.assertRaisesRegex(
      ...
      manager.add_bags([
          ...
          dbm.BagToAdd(bag_name='bag1', bag=bag1, dependencies=('bag2',)),
          dbm.BagToAdd(bag_name='bag2', bag=bag2, dependencies=('bag1',)),
          dbm.BagToAdd(bag_name='bag3', bag=bag3, dependencies=('bag1',)),
          dbm.BagToAdd(bag_name='bag4', bag=bag4, dependencies=('bag0',)),
      ])
  ...
  def test_create_branch(self):
    ...
    branch_manager = DataBagManager.create_from_dir(branch_dir)
    self.assertEqual(
    ...
    kd.testing.assert_equivalent(
        ...
        trunk_manager.get_minimal_bag(branch_manager.get_available_bag_names()),
    )
    ...
    self.assertEqual(
        DataBagManager.create_from_dir(trunk_dir).get_available_bag_names(),
        {'trunk1', 'trunk2', 'trunk3'},  # branch1 is not available
    )
    ...
    self.assertEqual(
        DataBagManager.create_from_dir(branch_dir).get_available_bag_names(),
        {'trunk1', 'trunk2', 'trunk3', 'branch1'},
    )
    ...
    twig_manager = DataBagManager.create_from_dir(twig_dir)
    self.assertEqual(
    ...
    another_branch_manager = DataBagManager.create_from_dir(another_branch_dir)
    self.assertEqual(
  ...
  def test_calling_constructor_directly_raises(self):
    with self.assertRaisesRegex(
        ValueError,
        re.escape(
            'please do not call the DataBagManager'
  ...
  def test_create_new_with_non_empty_dir(self):
    persistence_dir = self.create_tempdir().full_path
    with open(os.path.join(persistence_dir, 'foo'), 'w') as f:
      f.write('bar')
    with self.assertRaisesRegex(
        ValueError,
        re.escape('the persistence_dir must be empty or not exist yet. Got'),
    ):
      DataBagManager.create_new(persistence_dir)

  def test_create_from_dir_with_non_existent_dir(self):
    persistence_dir = os.path.join(
        self.create_tempdir().full_path, 'non_existent_dir'
    )
    with self.assertRaisesRegex(
        ValueError,
        re.escape('the persistence_dir does not exist. Got'),
    ):
      DataBagManager.create_from_dir(persistence_dir)

  def test_create_from_dir_with_empty_dir(self):
    persistence_dir = self.create_tempdir().full_path
    with self.assertRaisesRegex(
        ValueError,
        re.escape('the persistence_dir must not be empty. Got'),
    ):
      DataBagManager.create_from_dir(persistence_dir)

  def test_global_cache_entry_size_estimates(self):
    ...
    # persistence dir. It is associated in the global cache with the proper
    # size estimate.
    key = f'bag_name:bag1 filepath:{manager._get_bag_filepath("bag1")}'
    value_and_metadata = global_cache._cache[key]
