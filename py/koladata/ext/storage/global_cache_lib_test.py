...
from koladata.ext.storage import global_cache_flags
from koladata.ext.storage import global_cache_lib
from koladata.ext.storage import lru_size_tracking_cache
...
class GlobalCacheLibTest(absltest.TestCase):
  ...
  @flagsaver.flagsaver()
  def test_kd_ext_storage_global_cache_max_size_mb_flag_updates_cache(
      self,
  ):
    self.assertNotEqual(
        global_cache_lib.get_global_cache().get_max_total_bytes_of_entries_in_cache(),
    ...
    flags.FLAGS.kd_ext_storage_global_cache_max_size_mb = 42
    self.assertEqual(
        global_cache_lib.get_global_cache().get_max_total_bytes_of_entries_in_cache(),
        42 * 1024 * 1024,
    )
