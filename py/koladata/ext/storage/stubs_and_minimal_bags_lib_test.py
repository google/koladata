...
class StubsAndMinimalBagsLibTest(absltest.TestCase):
  ...
  def test_function_schema_stub(self):
    ...
    for struct_schema in [
        kd.schema.new_schema(x=kd.INT32, y=kd.INT64),  # pyrefly: ignore[missing-attribute]
        kd.schema.new_schema(x=kd.INT32, y=kd.INT64, z=kd.named_schema('foo')),  # pyrefly: ignore[missing-attribute]
        kd.dict_schema(kd.INT32, kd.INT64),  # pyrefly: ignore[missing-attribute]
        kd.dict_schema(kd.INT32, kd.named_schema('foo')),  # pyrefly: ignore[missing-attribute]
        kd.list_schema(kd.INT32),  # pyrefly: ignore[missing-attribute]
        kd.list_schema(kd.named_schema('foo')),  # pyrefly: ignore[missing-attribute]
    ]:
      kd.testing.assert_equivalent(
      ...
      struct_schema_with_metadata = struct_schema.enriched(
          kd.metadata(struct_schema, proto='my.proto.Message', version=123)  # pyrefly: ignore[missing-attribute]
      )
      ...
      expected_schema_stub = (
          ...
          .enriched(
              kd.attrs(  # pyrefly: ignore[missing-attribute]
                  ...
                  **{
                      '__schema_metadata__': kd.core.get_metadata(  # pyrefly: ignore[missing-attribute]
                          struct_schema_with_metadata
                      ),
                  }
              )
      ...
      kd.testing.assert_equivalent(
          kd.core.get_metadata(expected_schema_stub).extract(),  # pyrefly: ignore[missing-attribute]
          kd.core.get_metadata(struct_schema_with_metadata).extract(),  # pyrefly: ignore[missing-attribute]
      )

    # Named schemas retain their names in schema_stub():
    struct_schema_with_name = kd.named_schema('foo', x=kd.INT32, y=kd.INT64)  # pyrefly: ignore[missing-attribute]
    with self.assertRaises(AssertionError):
      kd.testing.assert_equivalent(
    ...
    kd.testing.assert_equivalent(
        kd.core.get_metadata(expected_schema_stub).extract(),  # pyrefly: ignore[missing-attribute]
        kd.core.get_metadata(struct_schema_with_name_and_metadata).extract(),  # pyrefly: ignore[missing-attribute]
    )
        ...
        stubs_and_minimal_bags_lib.schema_stub(
            struct_schema_with_name.enriched(
                kd.metadata(  # pyrefly: ignore[missing-attribute]
                    ...
                    proto=kd.named_schema('proto', name=kd.STRING),  # pyrefly: ignore[missing-attribute]
                )
            )
        )

  def test_minimal_bag_associating_list_with_its_items(self):
    item_schema = kd.named_schema('foo', bar=kd.INT32)  # pyrefly: ignore[missing-attribute]

    empty_list = kd.list([], item_schema=item_schema)  # pyrefly: ignore[missing-attribute]
    standard_list = kd.list([item_schema.new(bar=i) for i in range(3)])  # pyrefly: ignore[missing-attribute]
    list_with_no_info_about_items = kd.list(  # pyrefly: ignore[missing-attribute]
        [item_schema.new(bar=i + 5) for i in range(3)]
    ).no_bag()
    missing_list = None

    list_ds = kd.slice([  # pyrefly: ignore[missing-attribute]
        empty_list,
        standard_list,
      ...
      self.assertIsNone(
          stubs_and_minimal_bags_lib.minimal_bag_associating_list_with_its_items(
              kd.slice(  # pyrefly: ignore[missing-attribute]
                  ...
                  schema=kd.list_schema(item_schema),  # pyrefly: ignore[missing-attribute]
              )
          )
      )
    ...
    with self.subTest('with_items_that_are_schemas'):
      list_ds = kd.list([  # pyrefly: ignore[missing-attribute]
          kd.schema.new_schema(x=kd.INT32),  # pyrefly: ignore[missing-attribute]
          kd.INT32,
    ...
    with self.subTest('with_items_that_are_unsupported_schemas'):
      schema_item = kd.named_schema('foo', bar=kd.INT32)  # pyrefly: ignore[missing-attribute]
      schema_item = kd.with_metadata(schema_item, my_bad=kd.new(z='gotcha!'))  # pyrefly: ignore[missing-attribute]
      list_ds = kd.list([schema_item])  # pyrefly: ignore[missing-attribute]
      with self.assertRaisesRegex(
          ValueError,
          'schema .* has metadata attributes that are not primitives',
      ):
  ...
  def test_minimal_bag_associating_dict_with_its_keys_and_values(self):
    ...
    value_schema = kd.named_schema('DictValue', bar=kd.INT32)  # pyrefly: ignore[missing-attribute]
    dict_schema = kd.dict_schema(key_schema, value_schema)  # pyrefly: ignore[missing-attribute]

    empty_dict = kd.dict({}, schema=dict_schema)  # pyrefly: ignore[missing-attribute]
    value1 = value_schema.new(bar=1)
    value2 = value_schema.new(bar=2)
    standard_dict = kd.dict({'key1': value1, 'key2': value2})  # pyrefly: ignore[missing-attribute]
    dict_with_no_info_about_keys_and_values = kd.dict(  # pyrefly: ignore[missing-attribute]
        {'key3': value_schema.new(bar=3)}
    ).no_bag()
    missing_dict = None

    dict_ds = kd.slice([  # pyrefly: ignore[missing-attribute]
        empty_dict,
        standard_dict,
    ...
    kd.testing.assert_equivalent(
        ...
        kd.dicts.dict_update(  # pyrefly: ignore[missing-attribute]
            standard_dict,
            kd.slice(['key1', 'key2']),  # pyrefly: ignore[missing-attribute]
            kd.slice([value1.no_bag(), value2.no_bag()]),  # pyrefly: ignore[missing-attribute]
        ),
    )
    ...
    with self.subTest('works_for_shared_dicts_in_deep_dimensions'):
      value1 = kd.new(x=1, schema='foo')  # pyrefly: ignore[missing-attribute]
    ...
    with self.subTest('with_values_that_are_schemas'):
      dict_ds = kd.dict({  # pyrefly: ignore[missing-attribute]
          'key1': kd.schema.new_schema(x=kd.INT32),  # pyrefly: ignore[missing-attribute]
          'key2': kd.INT32,
    ...
    with self.subTest('with_values_that_are_unsupported_schemas'):
      schema_item = kd.named_schema('DictValue', bar=kd.INT32)  # pyrefly: ignore[missing-attribute]
      schema_item = kd.with_metadata(schema_item, my_bad=kd.new())  # pyrefly: ignore[missing-attribute]
      dict_ds = kd.dict({'key1': schema_item})  # pyrefly: ignore[missing-attribute]
      with self.assertRaisesRegex(
          ValueError,
          'schema .* has metadata attributes that are not primitives',
      ):
  ...
  def test_minimal_bag_associating_entity_with_its_attr_value(self):
    y_schema = kd.named_schema('Y', z=kd.INT32)  # pyrefly: ignore[missing-attribute]
    entity_schema = kd.named_schema('Entity', x=kd.INT32, y=y_schema)  # pyrefly: ignore[missing-attribute]
    ...
    entity_ds = kd.slice([  # pyrefly: ignore[missing-attribute]
        empty_entity,
        standard_entity,
    ...
    kd.testing.assert_equivalent(
        ...
        kd.attrs(standard_entity, y=y_value.no_bag()),  # pyrefly: ignore[missing-attribute]
    )
    ...
    with self.subTest('with_attr_values_that_are_schemas'):
      entity_schema = kd.schema.new_schema(s=kd.SCHEMA)  # pyrefly: ignore[missing-attribute]

      recursive_schema = kd.named_schema(  # pyrefly: ignore[missing-attribute]
          'TreeNode',
          x=kd.INT32,
      )
      recursive_schema = recursive_schema.with_attrs(
          children=kd.list_schema(recursive_schema)  # pyrefly: ignore[missing-attribute]
      )
      recursive_schema = recursive_schema.enriched(
          kd.metadata(recursive_schema, proto='my.proto.TreeNode', version=123),  # pyrefly: ignore[missing-attribute]
          kd.metadata(  # pyrefly: ignore[missing-attribute]
      ...
      entity_ds = kd.slice([  # pyrefly: ignore[missing-attribute]
          entity_schema.new(),
          entity_schema.new(s=kd.schema.new_schema(x=kd.INT32)),  # pyrefly: ignore[missing-attribute]
          entity_schema.new(s=kd.INT32),
          entity_schema.new(s=kd.dict_schema(kd.INT32, kd.INT64)),  # pyrefly: ignore[missing-attribute]
          entity_schema.new(s=kd.named_schema('foo', bar=kd.INT32)),  # pyrefly: ignore[missing-attribute]
    ...
    with self.subTest('with_attr_values_that_are_unsupported_schemas'):
      schema_item = kd.named_schema('Entity', bar=kd.INT32)  # pyrefly: ignore[missing-attribute]
      schema_item = kd.with_metadata(schema_item, my_bad=kd.new())  # pyrefly: ignore[missing-attribute]
      entity = kd.new(bar=schema_item)  # pyrefly: ignore[missing-attribute]
      with self.assertRaisesRegex(
          ValueError,
          'schema .* has metadata attributes that are not primitives',
      ):
      ...
      recursive_schema = recursive_schema.enriched(
          kd.metadata(  # pyrefly: ignore[missing-attribute]
              recursive_schema.children,
              non_primitive_metadata_attribute=kd.new(x=1),  # pyrefly: ignore[missing-attribute]
          ),
      )
      entity_ds = kd.slice([entity_schema.new(s=recursive_schema)])  # pyrefly: ignore[missing-attribute]
      with self.assertRaisesRegex(
          ValueError,
