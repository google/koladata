...
class AutoVariablesTest(absltest.TestCase):
  ...
  def test_nested_names(self):
    x = kde.with_name(kde.with_name(I.x, 'foo'), 'bar')  # pyrefly: ignore[missing-attribute]
  ...
  def test_existing_variables(self):
    x = ds([1, -2, 3, -4])
    y = ds([2, -2, -3, -4])
    shared = kde.with_name(x, 'foo')  # pyrefly: ignore[missing-attribute]
    fn = functor_factories.expr_fn(
        shared + kde.with_name(y, 'foo') + V.foo,  # pyrefly: ignore[missing-attribute]
        foo=V.foo_1[:] + shared,
  ...
  def test_does_not_lose_cast(self):
    fn = functor_factories.expr_fn(
        kde.slice(kdi.slice([1, 2, 3]), schema_constants.INT64),  # pyrefly: ignore[missing-attribute]
        auto_variables=True,
    )
  ...
  def test_dont_duplicate_computations(self):
    ...
    base = kde.py.apply_py(my_op, I.x)  # pyrefly: ignore[missing-attribute]
  ...
  def test_two_names_for_a_literal(self):
    ...
    testing.assert_equal(
        ...
        kde.explode(V._aux_0, ndim=1),  # pyrefly: ignore[missing-attribute]
    )
    testing.assert_equal(
  ...
  def test_shared_literal(self):
    ...
    testing.assert_equal(
        introspection.unpack_expr(fn.foo),
        kde.explode(V._aux_0, ndim=1) + 1,  # pyrefly: ignore[missing-attribute]
    )
    testing.assert_equal(
        introspection.unpack_expr(fn.bar),
        kde.explode(V._aux_0, ndim=1) + 2,  # pyrefly: ignore[missing-attribute]
    )
  ...
  def test_literal_with_source_location(self):
    fn = functor_factories.expr_fn(
        arolla.M.annotation.source_location(  # pyrefly: ignore[missing-attribute]
            kde.item(57),  # pyrefly: ignore[missing-attribute]
            arolla.namedtuple(
                function_name='test',
                file_name='test.py',
                line=1,
  ...
  def test_nested_duplicate_computations(self):
    ...
    expr = kde.py.apply_py(my_op, expr)  # pyrefly: ignore[missing-attribute]
  ...
  def test_extract_extra_nodes_literal_with_source_location(self):
    expr = arolla.M.annotation.source_location(  # pyrefly: ignore[missing-attribute]
        kde.item(57),  # pyrefly: ignore[missing-attribute]
        arolla.namedtuple(
            function_name='test',
            file_name='test.py',
  ...
  def test_extract_extra_nodes_kd_slice(self):
    expr = kde.slice(57)  # pyrefly: ignore[missing-attribute]
    fn = functor_factories.expr_fn(expr * I.x, auto_variables=False)
    res = _py_functors_py_ext.auto_variables(fn, [expr.fingerprint])
    testing.assert_equal(res(x=2), ds(114))
    testing.assert_equal(introspection.unpack_expr(res.returns), V._aux_0 * I.x)  # pyrefly: ignore[unsupported-operation]
  ...
  def test_extract_extra_nodes_kd_item(self):
    expr = kde.item(57)  # pyrefly: ignore[missing-attribute]
    fn = functor_factories.expr_fn(expr * I.x, auto_variables=False)
    res = _py_functors_py_ext.auto_variables(fn, [expr.fingerprint])
    testing.assert_equal(res(x=2), ds(114))
    testing.assert_equal(introspection.unpack_expr(res.returns), V._aux_0 * I.x)  # pyrefly: ignore[unsupported-operation]
