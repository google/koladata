...
class ExtensionTypesTest(parameterized.TestCase):
  ...
  def test_registry(self):
    ...
    with self.assertRaisesRegex(
      ...
      ext_types.register_extension_type(_MyOtherTestExtension, _EXT_TYPE)
  ...
  def test_attr_fields(self):
    ...
    with self.subTest('extra_field'):
      with self.assertRaisesRegex(
          TypeError, "MyExtensionWithDefaults\\(\\) got an unexpected keyword"
          " argument 'z'"
      ):
    ...
    with self.subTest('missing_field'):
      with self.assertRaisesRegex(
          TypeError,
          "MyExtensionWithDefaults\\(\\) missing 1 required keyword-only"
          " arguments: 'x'",
      ):
